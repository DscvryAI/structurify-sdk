/**
 * WH-347 Certified Payroll Data Extraction Example
 *
 * Extract structured data from wh-347 certified payroll documents using AI-powered extraction.
 *
 * Extract worker classifications, hours, wages, and compliance data from DOL WH-347 Certified Payroll forms
 *
 * Fields Extracted:
 *   - Contractor Name
 *   - Contractor Address
 ...
 * Try it online: https://structurify.ai/extract/wh-347-certified-payroll
 *
...
async function extractWh347CertifiedPayroll(filePath: string) {
  console.log('='.repeat(60));
  console.log('WH-347 Certified Payroll Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the WH-347 Certified Payroll template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'WH-347 Certified Payroll - SDK Example',
    templateId: 'ptpl_wh347_payroll',
  });
  ...
  // Save results
  const outputCsv = `wh-347-certified-payroll_extracted.csv`;
  const outputJson = `wh-347-certified-payroll_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'WH-347 Certified Payroll - Batch Processing',
    templateId: 'ptpl_wh347_payroll',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
WH-347 Certified Payroll Extraction Example
===========================================

Usage:
  npx ts-node wh-347-certified-payroll.ts <document-path>
  npx ts-node wh-347-certified-payroll.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node wh-347-certified-payroll.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/wh-347-certified-payroll
...
(async () => {
  try {
    if (args.length === 1) {
      await extractWh347CertifiedPayroll(args[0]);
    } else {
