/**
 * Uniform Residential Appraisal Report Data Extraction Example
 *
 * Extract structured data from uniform residential appraisal report documents using AI-powered extraction.
 *
 * Extract URAR form data for residential property appraisals
 *
 * Fields Extracted:
 *   - Property Address
 *   - Borrower Name
 ...
 * Try it online: https://structurify.ai/extract/uniform-residential-appraisal-report
 *
...
async function extractUniformResidentialAppraisalReport(filePath: string) {
  console.log('='.repeat(60));
  console.log('Uniform Residential Appraisal Report Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Uniform Residential Appraisal Report template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Uniform Residential Appraisal Report - SDK Example',
    templateId: 'ptpl_appraisal_urar',
  });
  ...
  // Save results
  const outputCsv = `uniform-residential-appraisal-report_extracted.csv`;
  const outputJson = `uniform-residential-appraisal-report_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Uniform Residential Appraisal Report - Batch Processing',
    templateId: 'ptpl_appraisal_urar',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Uniform Residential Appraisal Report Extraction Example
=======================================================

Usage:
  npx ts-node uniform-residential-appraisal-report.ts <document-path>
  npx ts-node uniform-residential-appraisal-report.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node uniform-residential-appraisal-report.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/uniform-residential-appraisal-report
...
(async () => {
  try {
    if (args.length === 1) {
      await extractUniformResidentialAppraisalReport(args[0]);
    } else {
