/**
 * California Real Estate Forms Data Extraction Example
 *
 * Extract structured data from california real estate forms documents using AI-powered extraction.
 *
 * Extract data from California real estate transaction forms
 *
 * Fields Extracted:
 *   - Form Type
 *   - Property Address
 ...
 *
 * Industry: Legal
 * Try it online: https://structurify.ai/extract/california-real-estate-forms
 *
...
async function extractCaliforniaRealEstateForms(filePath: string) {
  console.log('='.repeat(60));
  console.log('California Real Estate Forms Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the California Real Estate Forms template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'California Real Estate Forms - SDK Example',
    templateId: 'ptpl_ca_real_estate',
  });
  ...
  // Save results
  const outputCsv = `california-real-estate-forms_extracted.csv`;
  const outputJson = `california-real-estate-forms_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'California Real Estate Forms - Batch Processing',
    templateId: 'ptpl_ca_real_estate',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
California Real Estate Forms Extraction Example
===============================================

Usage:
  npx ts-node california-real-estate-forms.ts <document-path>
  npx ts-node california-real-estate-forms.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node california-real-estate-forms.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/california-real-estate-forms
...
(async () => {
  try {
    if (args.length === 1) {
      await extractCaliforniaRealEstateForms(args[0]);
    } else {
