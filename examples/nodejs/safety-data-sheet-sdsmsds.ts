/**
 * Safety Data Sheet (SDS/MSDS) Data Extraction Example
 *
 * Extract structured data from safety data sheet (sds/msds) documents using AI-powered extraction.
 *
 * Extract hazard and handling information from Safety Data Sheets for EHS compliance
 *
 * Fields Extracted:
 *   - Product Name
 *   - Product Code
 ...
 * Try it online: https://structurify.ai/extract/safety-data-sheet-sdsmsds
 *
...
async function extractSafetyDataSheetSdsmsds(filePath: string) {
  console.log('='.repeat(60));
  console.log('Safety Data Sheet (SDS/MSDS) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Safety Data Sheet (SDS/MSDS) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Safety Data Sheet (SDS/MSDS) - SDK Example',
    templateId: 'ptpl_sds',
  });
  ...
  // Save results
  const outputCsv = `safety-data-sheet-sdsmsds_extracted.csv`;
  const outputJson = `safety-data-sheet-sdsmsds_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Safety Data Sheet (SDS/MSDS) - Batch Processing',
    templateId: 'ptpl_sds',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Safety Data Sheet (SDS/MSDS) Extraction Example
===============================================

Usage:
  npx ts-node safety-data-sheet-sdsmsds.ts <document-path>
  npx ts-node safety-data-sheet-sdsmsds.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node safety-data-sheet-sdsmsds.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/safety-data-sheet-sdsmsds
...
(async () => {
  try {
    if (args.length === 1) {
      await extractSafetyDataSheetSdsmsds(args[0]);
    } else {
