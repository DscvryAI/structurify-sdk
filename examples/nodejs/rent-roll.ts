/**
 * Rent Roll Data Extraction Example
 *
 * Extract structured data from rent roll documents using AI-powered extraction.
 *
 * Extract tenant and rent data from property rent rolls for underwriting and valuation
 *
 * Fields Extracted:
 *   - Property Name
 *   - Property Address
 ...
 * Try it online: https://structurify.ai/extract/rent-roll
 *
...
async function extractRentRoll(filePath: string) {
  console.log('='.repeat(60));
  console.log('Rent Roll Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Rent Roll template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Rent Roll - SDK Example',
    templateId: 'ptpl_rent_roll',
  });
  ...
  // Save results
  const outputCsv = `rent-roll_extracted.csv`;
  const outputJson = `rent-roll_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Rent Roll - Batch Processing',
    templateId: 'ptpl_rent_roll',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Rent Roll Extraction Example
============================

Usage:
  npx ts-node rent-roll.ts <document-path>
  npx ts-node rent-roll.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node rent-roll.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/rent-roll
