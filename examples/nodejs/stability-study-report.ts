/**
 * Stability Study Report Data Extraction Example
 *
 * Extract structured data from stability study report documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/stability-study-report
 *
...
async function extractStabilityStudyReport(filePath: string) {
  console.log('='.repeat(60));
  console.log('Stability Study Report Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Stability Study Report template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Stability Study Report - SDK Example',
    templateId: 'ptpl_stability_study',
  });
  ...
  // Save results
  const outputCsv = `stability-study-report_extracted.csv`;
  const outputJson = `stability-study-report_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Stability Study Report - Batch Processing',
    templateId: 'ptpl_stability_study',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Stability Study Report Extraction Example
=========================================

Usage:
  npx ts-node stability-study-report.ts <document-path>
  npx ts-node stability-study-report.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node stability-study-report.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/stability-study-report
...
(async () => {
  try {
    if (args.length === 1) {
      await extractStabilityStudyReport(args[0]);
    } else {
