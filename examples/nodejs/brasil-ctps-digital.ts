/**
 * Brasil - CTPS Digital Data Extraction Example
 *
 * Extract structured data from brasil - ctps digital documents using AI-powered extraction.
 *
 * Extrair dados da Carteira de Trabalho e Previdência Social (CTPS) física ou digital
 *
 * Fields Extracted:
 *   - Nome
 *   - CPF
 ...
 *   - PIS/PASEP
 *
 * Industry: Identity Documents
 * Try it online: https://structurify.ai/extract/brasil-ctps-digital
 *
...
async function extractBrasilCtpsDigital(filePath: string) {
  console.log('='.repeat(60));
  console.log('Brasil - CTPS Digital Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Brasil - CTPS Digital template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Brasil - CTPS Digital - SDK Example',
    templateId: 'ptpl_brazil_ctps',
  });
  ...
  // Save results
  const outputCsv = `brasil-ctps-digital_extracted.csv`;
  const outputJson = `brasil-ctps-digital_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Brasil - CTPS Digital - Batch Processing',
    templateId: 'ptpl_brazil_ctps',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Brasil - CTPS Digital Extraction Example
========================================

Usage:
  npx ts-node brasil-ctps-digital.ts <document-path>
  npx ts-node brasil-ctps-digital.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node brasil-ctps-digital.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/brasil-ctps-digital
...
(async () => {
  try {
    if (args.length === 1) {
      await extractBrasilCtpsDigital(args[0]);
    } else {
