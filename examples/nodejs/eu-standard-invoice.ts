/**
 * EU Standard Invoice Data Extraction Example
 *
 * Extract structured data from eu standard invoice documents using AI-powered extraction.
 *
 * Extract data from European standard invoices
 *
 * Fields Extracted:
 *   - Supplier Name
 *   - Supplier VAT ID
 ...
 * Try it online: https://structurify.ai/extract/eu-standard-invoice
 *
...
async function extractEuStandardInvoice(filePath: string) {
  console.log('='.repeat(60));
  console.log('EU Standard Invoice Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the EU Standard Invoice template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'EU Standard Invoice - SDK Example',
    templateId: 'ptpl_eu_invoice',
  });
  ...
  // Save results
  const outputCsv = `eu-standard-invoice_extracted.csv`;
  const outputJson = `eu-standard-invoice_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'EU Standard Invoice - Batch Processing',
    templateId: 'ptpl_eu_invoice',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
EU Standard Invoice Extraction Example
======================================

Usage:
  npx ts-node eu-standard-invoice.ts <document-path>
  npx ts-node eu-standard-invoice.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node eu-standard-invoice.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/eu-standard-invoice
...
(async () => {
  try {
    if (args.length === 1) {
      await extractEuStandardInvoice(args[0]);
    } else {
