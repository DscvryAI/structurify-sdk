/**
 * Schedule K-1 (1065/1120S) Data Extraction Example
 *
 * Extract structured data from schedule k-1 (1065/1120s) documents using AI-powered extraction.
 *
 * Extract partner/shareholder income allocation from K-1 tax forms for tax preparation
 *
 * Fields Extracted:
 *   - Form Type
 *   - Tax Year
 *   - Partnership/Corp Name
 *   - Partnership/Corp EIN
 *   - Partnership/Corp Address
 ...
 *
 * Industry: Wealth Management / Tax
 * Try it online: https://structurify.ai/extract/schedule-k-1-10651120s
 *
...
async function extractScheduleK110651120S(filePath: string) {
  console.log('='.repeat(60));
  console.log('Schedule K-1 (1065/1120S) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Schedule K-1 (1065/1120S) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Schedule K-1 (1065/1120S) - SDK Example',
    templateId: 'ptpl_k1_form',
  });
  ...
  // Save results
  const outputCsv = `schedule-k-1-10651120s_extracted.csv`;
  const outputJson = `schedule-k-1-10651120s_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Schedule K-1 (1065/1120S) - Batch Processing',
    templateId: 'ptpl_k1_form',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Schedule K-1 (1065/1120S) Extraction Example
============================================

Usage:
  npx ts-node schedule-k-1-10651120s.ts <document-path>
  npx ts-node schedule-k-1-10651120s.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node schedule-k-1-10651120s.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/schedule-k-1-10651120s
...
(async () => {
  try {
    if (args.length === 1) {
      await extractScheduleK110651120S(args[0]);
    } else {
