/**
 * Medical Prescription Data Extraction Example
 *
 * Extract structured data from medical prescription documents using AI-powered extraction.
 *
 * Extract data from medical prescriptions
 *
 * Fields Extracted:
 *   - Patient Name
 *   - Patient DOB
 ...
 *
 * Industry: Healthcare
 * Try it online: https://structurify.ai/extract/medical-prescription
 *
...
async function extractMedicalPrescription(filePath: string) {
  console.log('='.repeat(60));
  console.log('Medical Prescription Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Medical Prescription template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Medical Prescription - SDK Example',
    templateId: 'ptpl_prescription',
  });
  ...
  // Save results
  const outputCsv = `medical-prescription_extracted.csv`;
  const outputJson = `medical-prescription_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Medical Prescription - Batch Processing',
    templateId: 'ptpl_prescription',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Medical Prescription Extraction Example
=======================================

Usage:
  npx ts-node medical-prescription.ts <document-path>
  npx ts-node medical-prescription.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node medical-prescription.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/medical-prescription
...
(async () => {
  try {
    if (args.length === 1) {
      await extractMedicalPrescription(args[0]);
    } else {
