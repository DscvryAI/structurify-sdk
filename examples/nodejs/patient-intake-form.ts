/**
 * Patient Intake Form Data Extraction Example
 *
 * Extract structured data from patient intake form documents using AI-powered extraction.
 *
 * Extract patient demographic and insurance information from intake forms for registration
 *
 * Fields Extracted:
 *   - Patient Last Name
 *   - Patient First Name
 ...
 *
 * Industry: Healthcare
 * Try it online: https://structurify.ai/extract/patient-intake-form
 *
...
async function extractPatientIntakeForm(filePath: string) {
  console.log('='.repeat(60));
  console.log('Patient Intake Form Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Patient Intake Form template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Patient Intake Form - SDK Example',
    templateId: 'ptpl_patient_intake',
  });
  ...
  // Save results
  const outputCsv = `patient-intake-form_extracted.csv`;
  const outputJson = `patient-intake-form_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Patient Intake Form - Batch Processing',
    templateId: 'ptpl_patient_intake',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Patient Intake Form Extraction Example
======================================

Usage:
  npx ts-node patient-intake-form.ts <document-path>
  npx ts-node patient-intake-form.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node patient-intake-form.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/patient-intake-form
...
(async () => {
  try {
    if (args.length === 1) {
      await extractPatientIntakeForm(args[0]);
    } else {
