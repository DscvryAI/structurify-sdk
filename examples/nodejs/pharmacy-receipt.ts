/**
 * Pharmacy Receipt Data Extraction Example
 *
 * Extract structured data from pharmacy receipt documents using AI-powered extraction.
 *
 * Extract data from pharmacy receipts and prescriptions
 *
 * Fields Extracted:
 *   - Pharmacy Name
 *   - Pharmacy Address
 ...
 *
 * Industry: Healthcare
 * Try it online: https://structurify.ai/extract/pharmacy-receipt
 *
...
async function extractPharmacyReceipt(filePath: string) {
  console.log('='.repeat(60));
  console.log('Pharmacy Receipt Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Pharmacy Receipt template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Pharmacy Receipt - SDK Example',
    templateId: 'ptpl_pharmacy_receipt',
  });
  ...
  // Save results
  const outputCsv = `pharmacy-receipt_extracted.csv`;
  const outputJson = `pharmacy-receipt_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Pharmacy Receipt - Batch Processing',
    templateId: 'ptpl_pharmacy_receipt',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Pharmacy Receipt Extraction Example
===================================

Usage:
  npx ts-node pharmacy-receipt.ts <document-path>
  npx ts-node pharmacy-receipt.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node pharmacy-receipt.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/pharmacy-receipt
...
(async () => {
  try {
    if (args.length === 1) {
      await extractPharmacyReceipt(args[0]);
    } else {
