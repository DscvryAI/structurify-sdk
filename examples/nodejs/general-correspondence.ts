/**
 * General Correspondence Data Extraction Example
 *
 * Extract structured data from general correspondence documents using AI-powered extraction.
 *
 * Extract metadata from letters and emails
 *
 * Fields Extracted:
 *   - From
 *   - To
 ...
 *
 * Industry: General
 * Try it online: https://structurify.ai/extract/general-correspondence
 *
...
async function extractGeneralCorrespondence(filePath: string) {
  console.log('='.repeat(60));
  console.log('General Correspondence Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the General Correspondence template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'General Correspondence - SDK Example',
    templateId: 'ptpl_correspondence',
  });
  ...
  // Save results
  const outputCsv = `general-correspondence_extracted.csv`;
  const outputJson = `general-correspondence_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'General Correspondence - Batch Processing',
    templateId: 'ptpl_correspondence',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
General Correspondence Extraction Example
=========================================

Usage:
  npx ts-node general-correspondence.ts <document-path>
  npx ts-node general-correspondence.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node general-correspondence.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/general-correspondence
...
(async () => {
  try {
    if (args.length === 1) {
      await extractGeneralCorrespondence(args[0]);
    } else {
