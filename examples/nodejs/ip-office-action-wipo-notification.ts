/**
 * IP Office Action & WIPO Notification Data Extraction Example
 *
 * Extract structured data from ip office action & wipo notification documents using AI-powered extraction.
 *
 ...
 *   - Rejections
 *
 * Industry: Legal Tech / IP
 * Try it online: https://structurify.ai/extract/ip-office-action-wipo-notification
 *
...
async function extractIpOfficeActionWipoNotification(filePath: string) {
  console.log('='.repeat(60));
  console.log('IP Office Action & WIPO Notification Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the IP Office Action & WIPO Notification template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'IP Office Action & WIPO Notification - SDK Example',
    templateId: 'ptpl_ip_office_action',
  });
  ...
  // Save results
  const outputCsv = `ip-office-action-wipo-notification_extracted.csv`;
  const outputJson = `ip-office-action-wipo-notification_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'IP Office Action & WIPO Notification - Batch Processing',
    templateId: 'ptpl_ip_office_action',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
IP Office Action & WIPO Notification Extraction Example
=======================================================

Usage:
  npx ts-node ip-office-action-wipo-notification.ts <document-path>
  npx ts-node ip-office-action-wipo-notification.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node ip-office-action-wipo-notification.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/ip-office-action-wipo-notification
...
(async () => {
  try {
    if (args.length === 1) {
      await extractIpOfficeActionWipoNotification(args[0]);
    } else {
