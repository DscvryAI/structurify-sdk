/**
 * Construction Change Order Data Extraction Example
 *
 * Extract structured data from construction change order documents using AI-powered extraction.
 *
 * Extract data from construction change orders and change directives
 *
 * Fields Extracted:
 *   - Change Order Number
 *   - PCO/RFP Number
 ...
 * Try it online: https://structurify.ai/extract/construction-change-order
 *
...
async function extractConstructionChangeOrder(filePath: string) {
  console.log('='.repeat(60));
  console.log('Construction Change Order Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Construction Change Order template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Construction Change Order - SDK Example',
    templateId: 'ptpl_change_order',
  });
  ...
  // Save results
  const outputCsv = `construction-change-order_extracted.csv`;
  const outputJson = `construction-change-order_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Construction Change Order - Batch Processing',
    templateId: 'ptpl_change_order',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Construction Change Order Extraction Example
============================================

Usage:
  npx ts-node construction-change-order.ts <document-path>
  npx ts-node construction-change-order.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node construction-change-order.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/construction-change-order
...
(async () => {
  try {
    if (args.length === 1) {
      await extractConstructionChangeOrder(args[0]);
    } else {
