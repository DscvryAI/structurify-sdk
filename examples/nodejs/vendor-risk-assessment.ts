/**
 * Vendor Risk Assessment Data Extraction Example
 *
 * Extract structured data from vendor risk assessment documents using AI-powered extraction.
 *
 * Comprehensive third-party vendor risk assessment from questionnaires, certifications, and financial documents
 *
 * Fields Extracted:
 *   - Vendor Name
 *   - Assessment Date
 ...
 * Try it online: https://structurify.ai/extract/vendor-risk-assessment
 *
...
async function extractVendorRiskAssessment(filePath: string) {
  console.log('='.repeat(60));
  console.log('Vendor Risk Assessment Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Vendor Risk Assessment template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Vendor Risk Assessment - SDK Example',
    templateId: 'ptpl_vendor_risk',
  });
  ...
  // Save results
  const outputCsv = `vendor-risk-assessment_extracted.csv`;
  const outputJson = `vendor-risk-assessment_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Vendor Risk Assessment - Batch Processing',
    templateId: 'ptpl_vendor_risk',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Vendor Risk Assessment Extraction Example
=========================================

Usage:
  npx ts-node vendor-risk-assessment.ts <document-path>
  npx ts-node vendor-risk-assessment.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node vendor-risk-assessment.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/vendor-risk-assessment
...
(async () => {
  try {
    if (args.length === 1) {
      await extractVendorRiskAssessment(args[0]);
    } else {
