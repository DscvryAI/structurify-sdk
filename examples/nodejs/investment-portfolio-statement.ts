/**
 * Investment Portfolio Statement Data Extraction Example
 *
 * Extract structured data from investment portfolio statement documents using AI-powered extraction.
 *
 * Extract data from brokerage and investment account statements
 *
 * Fields Extracted:
 *   - Account Holder
 *   - Account Number
 ...
 * Try it online: https://structurify.ai/extract/investment-portfolio-statement
 *
...
async function extractInvestmentPortfolioStatement(filePath: string) {
  console.log('='.repeat(60));
  console.log('Investment Portfolio Statement Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Investment Portfolio Statement template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Investment Portfolio Statement - SDK Example',
    templateId: 'ptpl_portfolio_statement',
  });
  ...
  // Save results
  const outputCsv = `investment-portfolio-statement_extracted.csv`;
  const outputJson = `investment-portfolio-statement_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Investment Portfolio Statement - Batch Processing',
    templateId: 'ptpl_portfolio_statement',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Investment Portfolio Statement Extraction Example
=================================================

Usage:
  npx ts-node investment-portfolio-statement.ts <document-path>
  npx ts-node investment-portfolio-statement.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node investment-portfolio-statement.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/investment-portfolio-statement
...
(async () => {
  try {
    if (args.length === 1) {
      await extractInvestmentPortfolioStatement(args[0]);
    } else {
