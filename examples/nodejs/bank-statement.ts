/**
 * Bank Statement Data Extraction Example
 *
 * Extract structured data from bank statement documents using AI-powered extraction.
 *
 * Extract account information and transactions from bank statements
 *
 * Fields Extracted:
 *   - Bank Name
 *   - Account Holder Name
 ...
 *
 * Industry: Financial
 * Try it online: https://structurify.ai/extract/bank-statement
 *
...
async function extractBankStatement(filePath: string) {
  ...
  console.log('Bank Statement Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Bank Statement template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Bank Statement - SDK Example',
    templateId: 'ptpl_bank_statement',
  });
  ...
  // Save results
  const outputCsv = `bank-statement_extracted.csv`;
  const outputJson = `bank-statement_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Bank Statement - Batch Processing',
    templateId: 'ptpl_bank_statement',
  });
  console.log(`Project created: ${project.id}`);
...
Usage:
  npx ts-node bank-statement.ts <document-path>
  npx ts-node bank-statement.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node bank-statement.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/bank-statement
...
(async () => {
  try {
    if (args.length === 1) {
      await extractBankStatement(args[0]);
    } else {
