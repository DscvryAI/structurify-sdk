/**
 * India Income Tax Return Data Extraction Example
 *
 * Extract structured data from india income tax return documents using AI-powered extraction.
 *
 * Extract data from Indian ITR forms
 *
 * Fields Extracted:
 *   - Assessment Year
 *   - PAN
 ...
 *
 * Industry: Government
 * Try it online: https://structurify.ai/extract/india-income-tax-return
 *
...
async function extractIndiaIncomeTaxReturn(filePath: string) {
  console.log('='.repeat(60));
  console.log('India Income Tax Return Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the India Income Tax Return template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'India Income Tax Return - SDK Example',
    templateId: 'ptpl_india_itr',
  });
  ...
  // Save results
  const outputCsv = `india-income-tax-return_extracted.csv`;
  const outputJson = `india-income-tax-return_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'India Income Tax Return - Batch Processing',
    templateId: 'ptpl_india_itr',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
India Income Tax Return Extraction Example
==========================================

Usage:
  npx ts-node india-income-tax-return.ts <document-path>
  npx ts-node india-income-tax-return.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node india-income-tax-return.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/india-income-tax-return
...
(async () => {
  try {
    if (args.length === 1) {
      await extractIndiaIncomeTaxReturn(args[0]);
    } else {
