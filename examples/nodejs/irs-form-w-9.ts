/**
 * IRS Form W-9 Data Extraction Example
 *
 * Extract structured data from irs form w-9 documents using AI-powered extraction.
 *
 * Extract data from W-9 Request for Taxpayer Identification Number forms
 *
 * Fields Extracted:
 *   - Name
 *   - Business Name
 ...
 * Try it online: https://structurify.ai/extract/irs-form-w-9
 *
...
async function extractIrsFormW9(filePath: string) {
  console.log('='.repeat(60));
  console.log('IRS Form W-9 Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the IRS Form W-9 template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'IRS Form W-9 - SDK Example',
    templateId: 'ptpl_w9_form',
  });
  ...
  // Save results
  const outputCsv = `irs-form-w-9_extracted.csv`;
  const outputJson = `irs-form-w-9_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'IRS Form W-9 - Batch Processing',
    templateId: 'ptpl_w9_form',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
IRS Form W-9 Extraction Example
===============================

Usage:
  npx ts-node irs-form-w-9.ts <document-path>
  npx ts-node irs-form-w-9.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node irs-form-w-9.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/irs-form-w-9
