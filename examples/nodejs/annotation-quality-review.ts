/**
 * Annotation Quality Review Data Extraction Example
 *
 * Extract structured data from annotation quality review documents using AI-powered extraction.
 *
 * Review and validate existing annotations for quality assurance and inter-annotator agreement
 *
 * Fields Extracted:
 *   - Original Label
 *   - Label Correct
 ...
 * Try it online: https://structurify.ai/extract/annotation-quality-review
 *
...
async function extractAnnotationQualityReview(filePath: string) {
  console.log('='.repeat(60));
  console.log('Annotation Quality Review Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Annotation Quality Review template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Annotation Quality Review - SDK Example',
    templateId: 'ptpl_annotation_qa',
  });
  ...
  // Save results
  const outputCsv = `annotation-quality-review_extracted.csv`;
  const outputJson = `annotation-quality-review_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Annotation Quality Review - Batch Processing',
    templateId: 'ptpl_annotation_qa',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Annotation Quality Review Extraction Example
============================================

Usage:
  npx ts-node annotation-quality-review.ts <document-path>
  npx ts-node annotation-quality-review.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node annotation-quality-review.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/annotation-quality-review
...
(async () => {
  try {
    if (args.length === 1) {
      await extractAnnotationQualityReview(args[0]);
    } else {
