/**
 * Well Log Header Data Extraction Example
 *
 * Extract structured data from well log header documents using AI-powered extraction.
 *
 * Extract metadata from well log headers (wireline logs, LWD, interpreted logs)
 *
 * Fields Extracted:
 *   - Well Name
 *   - Field Name
 ...
 *
 * Industry: Oil & Gas
 * Try it online: https://structurify.ai/extract/well-log-header
 *
...
async function extractWellLogHeader(filePath: string) {
  console.log('='.repeat(60));
  console.log('Well Log Header Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Well Log Header template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Well Log Header - SDK Example',
    templateId: 'ptpl_well_log_header',
  });
  ...
  // Save results
  const outputCsv = `well-log-header_extracted.csv`;
  const outputJson = `well-log-header_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Well Log Header - Batch Processing',
    templateId: 'ptpl_well_log_header',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Well Log Header Extraction Example
==================================

Usage:
  npx ts-node well-log-header.ts <document-path>
  npx ts-node well-log-header.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node well-log-header.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/well-log-header
...
(async () => {
  try {
    if (args.length === 1) {
      await extractWellLogHeader(args[0]);
    } else {
