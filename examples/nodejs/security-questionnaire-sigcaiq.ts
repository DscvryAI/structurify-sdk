/**
 * Security Questionnaire (SIG/CAIQ) Data Extraction Example
 *
 * Extract structured data from security questionnaire (sig/caiq) documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/security-questionnaire-sigcaiq
 *
...
async function extractSecurityQuestionnaireSigcaiq(filePath: string) {
  console.log('='.repeat(60));
  console.log('Security Questionnaire (SIG/CAIQ) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Security Questionnaire (SIG/CAIQ) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Security Questionnaire (SIG/CAIQ) - SDK Example',
    templateId: 'ptpl_security_questionnaire',
  });
  ...
  // Save results
  const outputCsv = `security-questionnaire-sigcaiq_extracted.csv`;
  const outputJson = `security-questionnaire-sigcaiq_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Security Questionnaire (SIG/CAIQ) - Batch Processing',
    templateId: 'ptpl_security_questionnaire',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Security Questionnaire (SIG/CAIQ) Extraction Example
====================================================

Usage:
  npx ts-node security-questionnaire-sigcaiq.ts <document-path>
  npx ts-node security-questionnaire-sigcaiq.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node security-questionnaire-sigcaiq.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/security-questionnaire-sigcaiq
...
(async () => {
  try {
    if (args.length === 1) {
      await extractSecurityQuestionnaireSigcaiq(args[0]);
    } else {
