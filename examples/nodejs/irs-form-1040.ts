/**
 * IRS Form 1040 Data Extraction Example
 *
 * Extract structured data from irs form 1040 documents using AI-powered extraction.
 *
 * Extract U.S. Individual Income Tax Return data
 *
 * Fields Extracted:
 *   - Tax Year
 *   - Taxpayer Name
 ...
 *
 * Industry: Government
 * Try it online: https://structurify.ai/extract/irs-form-1040
 *
...
async function extractIrsForm1040(filePath: string) {
  console.log('='.repeat(60));
  console.log('IRS Form 1040 Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the IRS Form 1040 template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'IRS Form 1040 - SDK Example',
    templateId: 'ptpl_irs_1040',
  });
  ...
  // Save results
  const outputCsv = `irs-form-1040_extracted.csv`;
  const outputJson = `irs-form-1040_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'IRS Form 1040 - Batch Processing',
    templateId: 'ptpl_irs_1040',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
IRS Form 1040 Extraction Example
================================

Usage:
  npx ts-node irs-form-1040.ts <document-path>
  npx ts-node irs-form-1040.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node irs-form-1040.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/irs-form-1040
...
(async () => {
  try {
    if (args.length === 1) {
      await extractIrsForm1040(args[0]);
    } else {
