/**
 * Utility Bill Data Extraction Example
 *
 * Extract structured data from utility bill documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/utility-bill
 *
...
async function extractUtilityBill(filePath: string) {
  console.log('='.repeat(60));
  console.log('Utility Bill Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Utility Bill template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Utility Bill - SDK Example',
    templateId: 'ptpl_utility_bill',
  });
  ...
  // Save results
  const outputCsv = `utility-bill_extracted.csv`;
  const outputJson = `utility-bill_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Utility Bill - Batch Processing',
    templateId: 'ptpl_utility_bill',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Utility Bill Extraction Example
===============================

Usage:
  npx ts-node utility-bill.ts <document-path>
  npx ts-node utility-bill.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node utility-bill.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/utility-bill
...
(async () => {
  try {
    if (args.length === 1) {
      await extractUtilityBill(args[0]);
    } else {
