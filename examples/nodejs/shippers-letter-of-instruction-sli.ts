/**
 * Shipper's Letter of Instruction (SLI) Data Extraction Example
 *
 * Extract structured data from shipper's letter of instruction (sli) documents using AI-powered extraction.
 *
 * Extract export documentation instructions from Shipper's Letters of Instruction for international shipments
 *
 * Fields Extracted:
 *   - SLI Date
 *   - Exporter Name
 ...
 *   - Ultimate Consignee
 *
 * Industry: Freight / Trade
 * Try it online: https://structurify.ai/extract/shippers-letter-of-instruction-sli
 *
...
async function extractShippersLetterOfInstructionSli(filePath: string) {
  console.log('='.repeat(60));
  console.log('Shipper's Letter of Instruction (SLI) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Shipper's Letter of Instruction (SLI) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Shipper's Letter of Instruction (SLI) - SDK Example',
    templateId: 'ptpl_shipper_letter',
  });
  ...
  // Save results
  const outputCsv = `shippers-letter-of-instruction-sli_extracted.csv`;
  const outputJson = `shippers-letter-of-instruction-sli_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Shipper's Letter of Instruction (SLI) - Batch Processing',
    templateId: 'ptpl_shipper_letter',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Shipper's Letter of Instruction (SLI) Extraction Example
========================================================

Usage:
  npx ts-node shippers-letter-of-instruction-sli.ts <document-path>
  npx ts-node shippers-letter-of-instruction-sli.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node shippers-letter-of-instruction-sli.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/shippers-letter-of-instruction-sli
...
(async () => {
  try {
    if (args.length === 1) {
      await extractShippersLetterOfInstructionSli(args[0]);
    } else {
