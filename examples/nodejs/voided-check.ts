/**
 * Voided Check Data Extraction Example
 *
 * Extract structured data from voided check documents using AI-powered extraction.
 *
 * Extract bank routing and account information from voided check images for payment setup
 *
 * Fields Extracted:
 *   - Bank Name
 *   - Bank Address
 ...
 *
 * Industry: Finance / HR
 * Try it online: https://structurify.ai/extract/voided-check
 *
...
async function extractVoidedCheck(filePath: string) {
  console.log('='.repeat(60));
  console.log('Voided Check Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Voided Check template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Voided Check - SDK Example',
    templateId: 'ptpl_voided_check',
  });
  ...
  // Save results
  const outputCsv = `voided-check_extracted.csv`;
  const outputJson = `voided-check_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Voided Check - Batch Processing',
    templateId: 'ptpl_voided_check',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Voided Check Extraction Example
===============================

Usage:
  npx ts-node voided-check.ts <document-path>
  npx ts-node voided-check.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node voided-check.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/voided-check
...
(async () => {
  try {
    if (args.length === 1) {
      await extractVoidedCheck(args[0]);
    } else {
