/**
 * GDPR Data Processing Agreement Data Extraction Example
 *
 * Extract structured data from gdpr data processing agreement documents using AI-powered extraction.
 *
 ...
 *
 * Industry: Privacy / Legal
 * Try it online: https://structurify.ai/extract/gdpr-data-processing-agreement
 *
...
async function extractGdprDataProcessingAgreement(filePath: string) {
  console.log('='.repeat(60));
  console.log('GDPR Data Processing Agreement Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the GDPR Data Processing Agreement template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'GDPR Data Processing Agreement - SDK Example',
    templateId: 'ptpl_gdpr_dpa',
  });
  ...
  // Save results
  const outputCsv = `gdpr-data-processing-agreement_extracted.csv`;
  const outputJson = `gdpr-data-processing-agreement_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'GDPR Data Processing Agreement - Batch Processing',
    templateId: 'ptpl_gdpr_dpa',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
GDPR Data Processing Agreement Extraction Example
=================================================

Usage:
  npx ts-node gdpr-data-processing-agreement.ts <document-path>
  npx ts-node gdpr-data-processing-agreement.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node gdpr-data-processing-agreement.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/gdpr-data-processing-agreement
...
(async () => {
  try {
    if (args.length === 1) {
      await extractGdprDataProcessingAgreement(args[0]);
    } else {
