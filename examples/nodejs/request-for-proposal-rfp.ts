/**
 * Request for Proposal (RFP) Data Extraction Example
 *
 * Extract structured data from request for proposal (rfp) documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/request-for-proposal-rfp
 *
...
async function extractRequestForProposalRfp(filePath: string) {
  console.log('='.repeat(60));
  console.log('Request for Proposal (RFP) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Request for Proposal (RFP) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Request for Proposal (RFP) - SDK Example',
    templateId: 'ptpl_rfp_document',
  });
  ...
  // Save results
  const outputCsv = `request-for-proposal-rfp_extracted.csv`;
  const outputJson = `request-for-proposal-rfp_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Request for Proposal (RFP) - Batch Processing',
    templateId: 'ptpl_rfp_document',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Request for Proposal (RFP) Extraction Example
=============================================

Usage:
  npx ts-node request-for-proposal-rfp.ts <document-path>
  npx ts-node request-for-proposal-rfp.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node request-for-proposal-rfp.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/request-for-proposal-rfp
...
(async () => {
  try {
    if (args.length === 1) {
      await extractRequestForProposalRfp(args[0]);
    } else {
