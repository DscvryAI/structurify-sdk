/**
 * Power of Attorney Data Extraction Example
 *
 * Extract structured data from power of attorney documents using AI-powered extraction.
 *
 * Extract parties, scope of authority, and terms from Power of Attorney documents
 *
 * Fields Extracted:
 *   - POA Type
 *   - Principal Name
 ...
 *
 * Industry: Legal
 * Try it online: https://structurify.ai/extract/power-of-attorney
 *
...
async function extractPowerOfAttorney(filePath: string) {
  console.log('='.repeat(60));
  console.log('Power of Attorney Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Power of Attorney template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Power of Attorney - SDK Example',
    templateId: 'ptpl_power_of_attorney',
  });
  ...
  // Save results
  const outputCsv = `power-of-attorney_extracted.csv`;
  const outputJson = `power-of-attorney_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Power of Attorney - Batch Processing',
    templateId: 'ptpl_power_of_attorney',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Power of Attorney Extraction Example
====================================

Usage:
  npx ts-node power-of-attorney.ts <document-path>
  npx ts-node power-of-attorney.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node power-of-attorney.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/power-of-attorney
...
(async () => {
  try {
    if (args.length === 1) {
      await extractPowerOfAttorney(args[0]);
    } else {
