/**
 * Payslip / Pay Stub Data Extraction Example
 *
 * Extract structured data from payslip / pay stub documents using AI-powered extraction.
 *
 * Extract data from employee payslips
 *
 * Fields Extracted:
 *   - Employee Name
 *   - Employee ID
 ...
 *
 * Industry: HR
 * Try it online: https://structurify.ai/extract/payslip-pay-stub
 *
...
async function extractPayslipPayStub(filePath: string) {
  console.log('='.repeat(60));
  console.log('Payslip / Pay Stub Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Payslip / Pay Stub template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Payslip / Pay Stub - SDK Example',
    templateId: 'ptpl_payslip',
  });
  ...
  // Save results
  const outputCsv = `payslip-pay-stub_extracted.csv`;
  const outputJson = `payslip-pay-stub_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Payslip / Pay Stub - Batch Processing',
    templateId: 'ptpl_payslip',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Payslip / Pay Stub Extraction Example
=====================================

Usage:
  npx ts-node payslip-pay-stub.ts <document-path>
  npx ts-node payslip-pay-stub.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node payslip-pay-stub.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/payslip-pay-stub
...
(async () => {
  try {
    if (args.length === 1) {
      await extractPayslipPayStub(args[0]);
    } else {
