/**
 * Commercial Invoice (Trade) Data Extraction Example
 *
 * Extract structured data from commercial invoice (trade) documents using AI-powered extraction.
 *
 * Extract data from commercial invoices for trade finance and customs
 *
 * Fields Extracted:
 *   - Invoice Number
 *   - Invoice Date
 ...
 * Try it online: https://structurify.ai/extract/commercial-invoice-trade
 *
...
async function extractCommercialInvoiceTrade(filePath: string) {
  console.log('='.repeat(60));
  console.log('Commercial Invoice (Trade) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Commercial Invoice (Trade) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Commercial Invoice (Trade) - SDK Example',
    templateId: 'ptpl_trade_invoice',
  });
  ...
  // Save results
  const outputCsv = `commercial-invoice-trade_extracted.csv`;
  const outputJson = `commercial-invoice-trade_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Commercial Invoice (Trade) - Batch Processing',
    templateId: 'ptpl_trade_invoice',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Commercial Invoice (Trade) Extraction Example
=============================================

Usage:
  npx ts-node commercial-invoice-trade.ts <document-path>
  npx ts-node commercial-invoice-trade.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node commercial-invoice-trade.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/commercial-invoice-trade
...
(async () => {
  try {
    if (args.length === 1) {
      await extractCommercialInvoiceTrade(args[0]);
    } else {
