/**
 * FATCA/CRS Self-Certification Data Extraction Example
 *
 * Extract structured data from fatca/crs self-certification documents using AI-powered extraction.
 *
 * Extract tax residency and status information from FATCA and Common Reporting Standard self-certification forms
 *
 * Fields Extracted:
 *   - Form Type
 *   - Account Holder Type
 ...
 * Try it online: https://structurify.ai/extract/fatcacrs-self-certification
 *
...
async function extractFatcacrsSelfCertification(filePath: string) {
  console.log('='.repeat(60));
  console.log('FATCA/CRS Self-Certification Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the FATCA/CRS Self-Certification template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'FATCA/CRS Self-Certification - SDK Example',
    templateId: 'ptpl_fatca_crs',
  });
  ...
  // Save results
  const outputCsv = `fatcacrs-self-certification_extracted.csv`;
  const outputJson = `fatcacrs-self-certification_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'FATCA/CRS Self-Certification - Batch Processing',
    templateId: 'ptpl_fatca_crs',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
FATCA/CRS Self-Certification Extraction Example
===============================================

Usage:
  npx ts-node fatcacrs-self-certification.ts <document-path>
  npx ts-node fatcacrs-self-certification.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node fatcacrs-self-certification.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/fatcacrs-self-certification
...
(async () => {
  try {
    if (args.length === 1) {
      await extractFatcacrsSelfCertification(args[0]);
    } else {
