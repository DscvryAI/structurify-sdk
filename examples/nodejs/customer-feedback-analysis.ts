/**
 * Customer Feedback Analysis Data Extraction Example
 *
 * Extract structured data from customer feedback analysis documents using AI-powered extraction.
 *
 * Analyze customer surveys, support tickets, reviews, and NPS responses for themes, sentiment, and actionable insights
 *
 * Fields Extracted:
 *   - Feedback Type
 *   - Source Platform
 ...
 * Try it online: https://structurify.ai/extract/customer-feedback-analysis
 *
...
async function extractCustomerFeedbackAnalysis(filePath: string) {
  console.log('='.repeat(60));
  console.log('Customer Feedback Analysis Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Customer Feedback Analysis template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Customer Feedback Analysis - SDK Example',
    templateId: 'ptpl_customer_feedback',
  });
  ...
  // Save results
  const outputCsv = `customer-feedback-analysis_extracted.csv`;
  const outputJson = `customer-feedback-analysis_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Customer Feedback Analysis - Batch Processing',
    templateId: 'ptpl_customer_feedback',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Customer Feedback Analysis Extraction Example
=============================================

Usage:
  npx ts-node customer-feedback-analysis.ts <document-path>
  npx ts-node customer-feedback-analysis.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node customer-feedback-analysis.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/customer-feedback-analysis
...
(async () => {
  try {
    if (args.length === 1) {
      await extractCustomerFeedbackAnalysis(args[0]);
    } else {
