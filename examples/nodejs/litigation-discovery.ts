/**
 * Litigation Discovery Data Extraction Example
 *
 * Extract structured data from litigation discovery documents using AI-powered extraction.
 *
 * Categorize and extract metadata from discovery documents
 *
 * Fields Extracted:
 *   - Document Type
 *   - Date
 ...
 *
 * Industry: Legal
 * Try it online: https://structurify.ai/extract/litigation-discovery
 *
...
async function extractLitigationDiscovery(filePath: string) {
  console.log('='.repeat(60));
  console.log('Litigation Discovery Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Litigation Discovery template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Litigation Discovery - SDK Example',
    templateId: 'ptpl_litigation',
  });
  ...
  // Save results
  const outputCsv = `litigation-discovery_extracted.csv`;
  const outputJson = `litigation-discovery_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Litigation Discovery - Batch Processing',
    templateId: 'ptpl_litigation',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Litigation Discovery Extraction Example
=======================================

Usage:
  npx ts-node litigation-discovery.ts <document-path>
  npx ts-node litigation-discovery.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node litigation-discovery.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/litigation-discovery
...
(async () => {
  try {
    if (args.length === 1) {
      await extractLitigationDiscovery(args[0]);
    } else {
