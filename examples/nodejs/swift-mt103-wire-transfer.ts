/**
 * SWIFT MT103 (Wire Transfer) Data Extraction Example
 *
 * Extract structured data from swift mt103 (wire transfer) documents using AI-powered extraction.
 *
 * Extract payment details from SWIFT MT103 single customer credit transfer messages and wire confirmations
 *
 * Fields Extracted:
 *   - Message Type
 *   - Sender Reference
 ...
 * Try it online: https://structurify.ai/extract/swift-mt103-wire-transfer
 *
...
async function extractSwiftMt103WireTransfer(filePath: string) {
  console.log('='.repeat(60));
  console.log('SWIFT MT103 (Wire Transfer) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the SWIFT MT103 (Wire Transfer) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'SWIFT MT103 (Wire Transfer) - SDK Example',
    templateId: 'ptpl_swift_mt103',
  });
  ...
  // Save results
  const outputCsv = `swift-mt103-wire-transfer_extracted.csv`;
  const outputJson = `swift-mt103-wire-transfer_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'SWIFT MT103 (Wire Transfer) - Batch Processing',
    templateId: 'ptpl_swift_mt103',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
SWIFT MT103 (Wire Transfer) Extraction Example
==============================================

Usage:
  npx ts-node swift-mt103-wire-transfer.ts <document-path>
  npx ts-node swift-mt103-wire-transfer.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node swift-mt103-wire-transfer.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/swift-mt103-wire-transfer
...
(async () => {
  try {
    if (args.length === 1) {
      await extractSwiftMt103WireTransfer(args[0]);
    } else {
