/**
 * Petrophysical Data Table Data Extraction Example
 *
 * Extract structured data from petrophysical data table documents using AI-powered extraction.
 *
 * Extract formation evaluation data from petrophysical summary tables (porosity, saturation, resistivity)
 *
 * Fields Extracted:
 *   - Well Name
 *   - Data Type
 ...
 *
 * Industry: Oil & Gas
 * Try it online: https://structurify.ai/extract/petrophysical-data-table
 *
...
async function extractPetrophysicalDataTable(filePath: string) {
  console.log('='.repeat(60));
  console.log('Petrophysical Data Table Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Petrophysical Data Table template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Petrophysical Data Table - SDK Example',
    templateId: 'ptpl_petrophysical_data',
  });
  ...
  // Save results
  const outputCsv = `petrophysical-data-table_extracted.csv`;
  const outputJson = `petrophysical-data-table_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Petrophysical Data Table - Batch Processing',
    templateId: 'ptpl_petrophysical_data',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Petrophysical Data Table Extraction Example
===========================================

Usage:
  npx ts-node petrophysical-data-table.ts <document-path>
  npx ts-node petrophysical-data-table.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node petrophysical-data-table.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/petrophysical-data-table
...
(async () => {
  try {
    if (args.length === 1) {
      await extractPetrophysicalDataTable(args[0]);
    } else {
