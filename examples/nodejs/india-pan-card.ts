/**
 * India PAN Card Data Extraction Example
 *
 * Extract structured data from india pan card documents using AI-powered extraction.
 *
 * Extract data from Indian PAN cards
 *
 * Fields Extracted:
 *   - PAN Number
 *   - Name
 ...
 * Try it online: https://structurify.ai/extract/india-pan-card
 *
...
async function extractIndiaPanCard(filePath: string) {
  console.log('='.repeat(60));
  console.log('India PAN Card Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the India PAN Card template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'India PAN Card - SDK Example',
    templateId: 'ptpl_pan_card_india',
  });
  ...
  // Save results
  const outputCsv = `india-pan-card_extracted.csv`;
  const outputJson = `india-pan-card_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'India PAN Card - Batch Processing',
    templateId: 'ptpl_pan_card_india',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
India PAN Card Extraction Example
=================================

Usage:
  npx ts-node india-pan-card.ts <document-path>
  npx ts-node india-pan-card.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node india-pan-card.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/india-pan-card
...
(async () => {
  try {
    if (args.length === 1) {
      await extractIndiaPanCard(args[0]);
    } else {
