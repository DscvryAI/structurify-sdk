/**
 * ACORD 2 - Automobile Loss Notice Data Extraction Example
 *
 * Extract structured data from acord 2 - automobile loss notice documents using AI-powered extraction.
 *
 * Extract data from ACORD 2 auto loss/claim notices
 *
 * Fields Extracted:
 *   - Insured Name
 *   - Policy Number
 ...
 *
 * Industry: Insurance
 * Try it online: https://structurify.ai/extract/acord-2-automobile-loss-notice
 *
...
async function extractAcord2AutomobileLossNotice(filePath: string) {
  console.log('='.repeat(60));
  console.log('ACORD 2 - Automobile Loss Notice Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the ACORD 2 - Automobile Loss Notice template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'ACORD 2 - Automobile Loss Notice - SDK Example',
    templateId: 'ptpl_acord_2',
  });
  ...
  // Save results
  const outputCsv = `acord-2-automobile-loss-notice_extracted.csv`;
  const outputJson = `acord-2-automobile-loss-notice_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'ACORD 2 - Automobile Loss Notice - Batch Processing',
    templateId: 'ptpl_acord_2',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
ACORD 2 - Automobile Loss Notice Extraction Example
===================================================

Usage:
  npx ts-node acord-2-automobile-loss-notice.ts <document-path>
  npx ts-node acord-2-automobile-loss-notice.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node acord-2-automobile-loss-notice.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/acord-2-automobile-loss-notice
...
(async () => {
  try {
    if (args.length === 1) {
      await extractAcord2AutomobileLossNotice(args[0]);
    } else {
