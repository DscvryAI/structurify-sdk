/**
 * US Customs Entry Summary 7501 Data Extraction Example
 *
 * Extract structured data from us customs entry summary 7501 documents using AI-powered extraction.
 *
 * Extract data from CBP Form 7501 customs entry
 *
 * Fields Extracted:
 *   - Entry Number
 *   - Entry Type
 ...
 *
 * Industry: Government
 * Try it online: https://structurify.ai/extract/us-customs-entry-summary-7501
 *
...
async function extractUsCustomsEntrySummary7501(filePath: string) {
  console.log('='.repeat(60));
  console.log('US Customs Entry Summary 7501 Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the US Customs Entry Summary 7501 template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'US Customs Entry Summary 7501 - SDK Example',
    templateId: 'ptpl_cbp_7501',
  });
  ...
  // Save results
  const outputCsv = `us-customs-entry-summary-7501_extracted.csv`;
  const outputJson = `us-customs-entry-summary-7501_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'US Customs Entry Summary 7501 - Batch Processing',
    templateId: 'ptpl_cbp_7501',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
US Customs Entry Summary 7501 Extraction Example
================================================

Usage:
  npx ts-node us-customs-entry-summary-7501.ts <document-path>
  npx ts-node us-customs-entry-summary-7501.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node us-customs-entry-summary-7501.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/us-customs-entry-summary-7501
...
(async () => {
  try {
    if (args.length === 1) {
      await extractUsCustomsEntrySummary7501(args[0]);
    } else {
