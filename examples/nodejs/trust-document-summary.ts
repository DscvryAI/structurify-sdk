/**
 * Trust Document Summary Data Extraction Example
 *
 * Extract structured data from trust document summary documents using AI-powered extraction.
 *
 * Extract key terms from trust agreements and trust documents
 *
 * Fields Extracted:
 *   - Trust Name
 *   - Trust Type
 ...
 *   - Distribution Terms
 *
 * Industry: Wealth Management / Estate
 * Try it online: https://structurify.ai/extract/trust-document-summary
 *
...
async function extractTrustDocumentSummary(filePath: string) {
  console.log('='.repeat(60));
  console.log('Trust Document Summary Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Trust Document Summary template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Trust Document Summary - SDK Example',
    templateId: 'ptpl_trust_document',
  });
  ...
  // Save results
  const outputCsv = `trust-document-summary_extracted.csv`;
  const outputJson = `trust-document-summary_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Trust Document Summary - Batch Processing',
    templateId: 'ptpl_trust_document',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Trust Document Summary Extraction Example
=========================================

Usage:
  npx ts-node trust-document-summary.ts <document-path>
  npx ts-node trust-document-summary.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node trust-document-summary.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/trust-document-summary
...
(async () => {
  try {
    if (args.length === 1) {
      await extractTrustDocumentSummary(args[0]);
    } else {
