/**
 * Delivery Note Data Extraction Example
 *
 * Extract structured data from delivery note documents using AI-powered extraction.
 *
 * Extract data from delivery notes and proof of delivery
 *
 * Fields Extracted:
 ...
 * Try it online: https://structurify.ai/extract/delivery-note
 *
...
async function extractDeliveryNote(filePath: string) {
  console.log('='.repeat(60));
  console.log('Delivery Note Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Delivery Note template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Delivery Note - SDK Example',
    templateId: 'ptpl_delivery_note',
  });
  ...
  // Save results
  const outputCsv = `delivery-note_extracted.csv`;
  const outputJson = `delivery-note_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Delivery Note - Batch Processing',
    templateId: 'ptpl_delivery_note',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Delivery Note Extraction Example
================================

Usage:
  npx ts-node delivery-note.ts <document-path>
  npx ts-node delivery-note.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node delivery-note.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/delivery-note
...
(async () => {
  try {
    if (args.length === 1) {
      await extractDeliveryNote(args[0]);
    } else {
