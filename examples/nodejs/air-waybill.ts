/**
 * Air Waybill Data Extraction Example
 *
 * Extract structured data from air waybill documents using AI-powered extraction.
 *
 * Extract data from air waybills (AWB)
 *
 * Fields Extracted:
 *   - AWB Number
 *   - Airline Code
 ...
 * Try it online: https://structurify.ai/extract/air-waybill
 *
...
async function extractAirWaybill(filePath: string) {
  console.log('='.repeat(60));
  console.log('Air Waybill Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Air Waybill template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Air Waybill - SDK Example',
    templateId: 'ptpl_air_waybill',
  });
  ...
  // Save results
  const outputCsv = `air-waybill_extracted.csv`;
  const outputJson = `air-waybill_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Air Waybill - Batch Processing',
    templateId: 'ptpl_air_waybill',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Air Waybill Extraction Example
==============================

Usage:
  npx ts-node air-waybill.ts <document-path>
  npx ts-node air-waybill.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node air-waybill.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/air-waybill
...
(async () => {
  try {
    if (args.length === 1) {
      await extractAirWaybill(args[0]);
    } else {
