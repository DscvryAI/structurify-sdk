/**
 * Bill of Exchange / Draft Data Extraction Example
 *
 * Extract structured data from bill of exchange / draft documents using AI-powered extraction.
 *
 * Extract data from bills of exchange and trade drafts
 *
 * Fields Extracted:
 *   - Draft Number
 *   - Draft Type
 ...
 * Try it online: https://structurify.ai/extract/bill-of-exchange-draft
 *
...
async function extractBillOfExchangeDraft(filePath: string) {
  console.log('='.repeat(60));
  console.log('Bill of Exchange / Draft Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Bill of Exchange / Draft template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Bill of Exchange / Draft - SDK Example',
    templateId: 'ptpl_bill_of_exchange',
  });
  ...
  // Save results
  const outputCsv = `bill-of-exchange-draft_extracted.csv`;
  const outputJson = `bill-of-exchange-draft_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Bill of Exchange / Draft - Batch Processing',
    templateId: 'ptpl_bill_of_exchange',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Bill of Exchange / Draft Extraction Example
===========================================

Usage:
  npx ts-node bill-of-exchange-draft.ts <document-path>
  npx ts-node bill-of-exchange-draft.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node bill-of-exchange-draft.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/bill-of-exchange-draft
...
(async () => {
  try {
    if (args.length === 1) {
      await extractBillOfExchangeDraft(args[0]);
    } else {
