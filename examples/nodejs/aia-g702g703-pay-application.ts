/**
 * AIA G702/G703 - Pay Application Data Extraction Example
 *
 * Extract structured data from aia g702/g703 - pay application documents using AI-powered extraction.
 *
 * Extract payment application data from AIA G702 (Application) and G703 (Continuation Sheet) forms
 *
 * Fields Extracted:
 *   - Application Number
 *   - Period To
 ...
 * Try it online: https://structurify.ai/extract/aia-g702g703-pay-application
 *
...
async function extractAiaG702G703PayApplication(filePath: string) {
  console.log('='.repeat(60));
  console.log('AIA G702/G703 - Pay Application Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the AIA G702/G703 - Pay Application template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'AIA G702/G703 - Pay Application - SDK Example',
    templateId: 'ptpl_aia_pay_application',
  });
  ...
  // Save results
  const outputCsv = `aia-g702g703-pay-application_extracted.csv`;
  const outputJson = `aia-g702g703-pay-application_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'AIA G702/G703 - Pay Application - Batch Processing',
    templateId: 'ptpl_aia_pay_application',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
AIA G702/G703 - Pay Application Extraction Example
==================================================

Usage:
  npx ts-node aia-g702g703-pay-application.ts <document-path>
  npx ts-node aia-g702g703-pay-application.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node aia-g702g703-pay-application.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/aia-g702g703-pay-application
...
(async () => {
  try {
    if (args.length === 1) {
      await extractAiaG702G703PayApplication(args[0]);
    } else {
