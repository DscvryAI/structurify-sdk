/**
 * Laboratory Results Data Extraction Example
 *
 * Extract structured data from laboratory results documents using AI-powered extraction.
 *
 * Extract data from medical lab test results
 *
 * Fields Extracted:
 *   - Patient Name
 *   - Patient ID
 ...
 *
 * Industry: Healthcare
 * Try it online: https://structurify.ai/extract/laboratory-results
 *
...
async function extractLaboratoryResults(filePath: string) {
  console.log('='.repeat(60));
  console.log('Laboratory Results Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Laboratory Results template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Laboratory Results - SDK Example',
    templateId: 'ptpl_lab_results',
  });
  ...
  // Save results
  const outputCsv = `laboratory-results_extracted.csv`;
  const outputJson = `laboratory-results_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Laboratory Results - Batch Processing',
    templateId: 'ptpl_lab_results',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Laboratory Results Extraction Example
=====================================

Usage:
  npx ts-node laboratory-results.ts <document-path>
  npx ts-node laboratory-results.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node laboratory-results.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/laboratory-results
...
(async () => {
  try {
    if (args.length === 1) {
      await extractLaboratoryResults(args[0]);
    } else {
