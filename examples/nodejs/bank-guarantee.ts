/**
 * Bank Guarantee Data Extraction Example
 *
 * Extract structured data from bank guarantee documents using AI-powered extraction.
 *
 * Extract data from bank guarantees and standby letters of credit
 *
 * Fields Extracted:
 *   - Guarantee Number
 *   - Guarantee Type
 ...
 * Try it online: https://structurify.ai/extract/bank-guarantee
 *
...
async function extractBankGuarantee(filePath: string) {
  console.log('='.repeat(60));
  console.log('Bank Guarantee Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Bank Guarantee template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Bank Guarantee - SDK Example',
    templateId: 'ptpl_bank_guarantee',
  });
  ...
  // Save results
  const outputCsv = `bank-guarantee_extracted.csv`;
  const outputJson = `bank-guarantee_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Bank Guarantee - Batch Processing',
    templateId: 'ptpl_bank_guarantee',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Bank Guarantee Extraction Example
=================================

Usage:
  npx ts-node bank-guarantee.ts <document-path>
  npx ts-node bank-guarantee.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node bank-guarantee.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/bank-guarantee
...
(async () => {
  try {
    if (args.length === 1) {
      await extractBankGuarantee(args[0]);
    } else {
