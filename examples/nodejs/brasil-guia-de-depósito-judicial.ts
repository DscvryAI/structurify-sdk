/**
 * Brasil - Guia de Depósito Judicial Data Extraction Example
 *
 * Extract structured data from brasil - guia de depósito judicial documents using AI-powered extraction.
 *
 * Extrair dados de guias de depósito judicial/recursal para controle de processos trabalhistas
 *
 * Fields Extracted:
 *   - Número do Processo
 *   - Tipo de Depósito
 ...
 *
 * Industry: Legal
 * Try it online: https://structurify.ai/extract/brasil-guia-de-depósito-judicial
 *
...
async function extractBrasilGuiaDeDepósitoJudicial(filePath: string) {
  console.log('='.repeat(60));
  console.log('Brasil - Guia de Depósito Judicial Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Brasil - Guia de Depósito Judicial template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Brasil - Guia de Depósito Judicial - SDK Example',
    templateId: 'ptpl_brazil_judicial_deposit',
  });
  ...
  // Save results
  const outputCsv = `brasil-guia-de-depósito-judicial_extracted.csv`;
  const outputJson = `brasil-guia-de-depósito-judicial_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Brasil - Guia de Depósito Judicial - Batch Processing',
    templateId: 'ptpl_brazil_judicial_deposit',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Brasil - Guia de Depósito Judicial Extraction Example
=====================================================

Usage:
  npx ts-node brasil-guia-de-depósito-judicial.ts <document-path>
  npx ts-node brasil-guia-de-depósito-judicial.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node brasil-guia-de-depósito-judicial.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/brasil-guia-de-depósito-judicial
...
(async () => {
  try {
    if (args.length === 1) {
      await extractBrasilGuiaDeDepósitoJudicial(args[0]);
    } else {
