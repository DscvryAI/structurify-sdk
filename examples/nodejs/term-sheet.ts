/**
 * Term Sheet Data Extraction Example
 *
 ...
 * Try it online: https://structurify.ai/extract/term-sheet
 *
...
async function extractTermSheet(filePath: string) {
  console.log('='.repeat(60));
  console.log('Term Sheet Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Term Sheet template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Term Sheet - SDK Example',
    templateId: 'ptpl_term_sheet',
  });
  ...
  // Save results
  const outputCsv = `term-sheet_extracted.csv`;
  const outputJson = `term-sheet_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Term Sheet - Batch Processing',
    templateId: 'ptpl_term_sheet',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Term Sheet Extraction Example
=============================

Usage:
  npx ts-node term-sheet.ts <document-path>
  npx ts-node term-sheet.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node term-sheet.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/term-sheet
