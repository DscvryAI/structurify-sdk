/**
 * ISO 27001 Statement of Applicability Data Extraction Example
 *
 * Extract structured data from iso 27001 statement of applicability documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/iso-27001-statement-of-applicability
 *
...
async function extractIso27001StatementOfApplicability(filePath: string) {
  console.log('='.repeat(60));
  console.log('ISO 27001 Statement of Applicability Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the ISO 27001 Statement of Applicability template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'ISO 27001 Statement of Applicability - SDK Example',
    templateId: 'ptpl_iso27001_soa',
  });
  ...
  // Save results
  const outputCsv = `iso-27001-statement-of-applicability_extracted.csv`;
  const outputJson = `iso-27001-statement-of-applicability_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'ISO 27001 Statement of Applicability - Batch Processing',
    templateId: 'ptpl_iso27001_soa',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
ISO 27001 Statement of Applicability Extraction Example
=======================================================

Usage:
  npx ts-node iso-27001-statement-of-applicability.ts <document-path>
  npx ts-node iso-27001-statement-of-applicability.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node iso-27001-statement-of-applicability.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/iso-27001-statement-of-applicability
...
(async () => {
  try {
    if (args.length === 1) {
      await extractIso27001StatementOfApplicability(args[0]);
    } else {
