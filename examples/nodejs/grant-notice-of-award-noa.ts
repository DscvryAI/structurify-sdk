/**
 * Grant Notice of Award (NOA) Data Extraction Example
 *
 * Extract structured data from grant notice of award (noa) documents using AI-powered extraction.
 *
 * Extract funding amounts, dates, and terms from federal Grant Notices of Award (NIH, NSF, DOE, etc.)
 *
 * Fields Extracted:
 *   - Award Number
 *   - FAIN
 ...
 *
 * Industry: Research Administration
 * Try it online: https://structurify.ai/extract/grant-notice-of-award-noa
 *
...
async function extractGrantNoticeOfAwardNoa(filePath: string) {
  console.log('='.repeat(60));
  console.log('Grant Notice of Award (NOA) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Grant Notice of Award (NOA) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Grant Notice of Award (NOA) - SDK Example',
    templateId: 'ptpl_grant_noa',
  });
  ...
  // Save results
  const outputCsv = `grant-notice-of-award-noa_extracted.csv`;
  const outputJson = `grant-notice-of-award-noa_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Grant Notice of Award (NOA) - Batch Processing',
    templateId: 'ptpl_grant_noa',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Grant Notice of Award (NOA) Extraction Example
==============================================

Usage:
  npx ts-node grant-notice-of-award-noa.ts <document-path>
  npx ts-node grant-notice-of-award-noa.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node grant-notice-of-award-noa.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/grant-notice-of-award-noa
...
(async () => {
  try {
    if (args.length === 1) {
      await extractGrantNoticeOfAwardNoa(args[0]);
    } else {
