/**
 * KYC - Corporate/Entity Data Extraction Example
 *
 * Extract structured data from kyc - corporate/entity documents using AI-powered extraction.
 *
 * Extract data from Know Your Customer documents for corporate/entity clients
 *
 * Fields Extracted:
 *   - Legal Entity Name
 *   - Trading Name
 ...
 * Try it online: https://structurify.ai/extract/kyc-corporateentity
 *
...
async function extractKycCorporateentity(filePath: string) {
  console.log('='.repeat(60));
  console.log('KYC - Corporate/Entity Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the KYC - Corporate/Entity template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'KYC - Corporate/Entity - SDK Example',
    templateId: 'ptpl_kyc_entity',
  });
  ...
  // Save results
  const outputCsv = `kyc-corporateentity_extracted.csv`;
  const outputJson = `kyc-corporateentity_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'KYC - Corporate/Entity - Batch Processing',
    templateId: 'ptpl_kyc_entity',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
KYC - Corporate/Entity Extraction Example
=========================================

Usage:
  npx ts-node kyc-corporateentity.ts <document-path>
  npx ts-node kyc-corporateentity.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node kyc-corporateentity.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/kyc-corporateentity
...
(async () => {
  try {
    if (args.length === 1) {
      await extractKycCorporateentity(args[0]);
    } else {
