/**
 * EU National ID Card Data Extraction Example
 *
 * Extract structured data from eu national id card documents using AI-powered extraction.
 *
 * Extract data from European national ID cards
 *
 * Fields Extracted:
 *   - Country
 *   - Document Number
 ...
 * Try it online: https://structurify.ai/extract/eu-national-id-card
 *
...
async function extractEuNationalIdCard(filePath: string) {
  console.log('='.repeat(60));
  console.log('EU National ID Card Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the EU National ID Card template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'EU National ID Card - SDK Example',
    templateId: 'ptpl_eu_id_card',
  });
  ...
  // Save results
  const outputCsv = `eu-national-id-card_extracted.csv`;
  const outputJson = `eu-national-id-card_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'EU National ID Card - Batch Processing',
    templateId: 'ptpl_eu_id_card',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
EU National ID Card Extraction Example
======================================

Usage:
  npx ts-node eu-national-id-card.ts <document-path>
  npx ts-node eu-national-id-card.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node eu-national-id-card.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/eu-national-id-card
...
(async () => {
  try {
    if (args.length === 1) {
      await extractEuNationalIdCard(args[0]);
    } else {
