/**
 * CAM Reconciliation Data Extraction Example
 *
 * Extract structured data from cam reconciliation documents using AI-powered extraction.
 *
 * Extract Common Area Maintenance reconciliation data for tenant billing and audits
 *
 * Fields Extracted:
 *   - Property Name
 *   - Property Address
 ...
 * Try it online: https://structurify.ai/extract/cam-reconciliation
 *
...
async function extractCamReconciliation(filePath: string) {
  console.log('='.repeat(60));
  console.log('CAM Reconciliation Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the CAM Reconciliation template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'CAM Reconciliation - SDK Example',
    templateId: 'ptpl_cam_reconciliation',
  });
  ...
  // Save results
  const outputCsv = `cam-reconciliation_extracted.csv`;
  const outputJson = `cam-reconciliation_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'CAM Reconciliation - Batch Processing',
    templateId: 'ptpl_cam_reconciliation',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
CAM Reconciliation Extraction Example
=====================================

Usage:
  npx ts-node cam-reconciliation.ts <document-path>
  npx ts-node cam-reconciliation.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node cam-reconciliation.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/cam-reconciliation
...
(async () => {
  try {
    if (args.length === 1) {
      await extractCamReconciliation(args[0]);
    } else {
