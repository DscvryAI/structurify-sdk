/**
 * Visa Document Data Extraction Example
 *
 * Extract structured data from visa document documents using AI-powered extraction.
 *
 * Extract data from visa stickers and documents
 *
 * Fields Extracted:
 *   - Visa Type
 *   - Issuing Country
 ...
 * Try it online: https://structurify.ai/extract/visa-document
 *
...
async function extractVisaDocument(filePath: string) {
  console.log('='.repeat(60));
  console.log('Visa Document Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Visa Document template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Visa Document - SDK Example',
    templateId: 'ptpl_visa',
  });
  ...
  // Save results
  const outputCsv = `visa-document_extracted.csv`;
  const outputJson = `visa-document_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Visa Document - Batch Processing',
    templateId: 'ptpl_visa',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Visa Document Extraction Example
================================

Usage:
  npx ts-node visa-document.ts <document-path>
  npx ts-node visa-document.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node visa-document.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/visa-document
...
(async () => {
  try {
    if (args.length === 1) {
      await extractVisaDocument(args[0]);
    } else {
