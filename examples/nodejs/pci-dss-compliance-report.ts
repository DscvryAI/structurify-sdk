/**
 * PCI DSS Compliance Report Data Extraction Example
 *
 * Extract structured data from pci dss compliance report documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/pci-dss-compliance-report
 *
...
async function extractPciDssComplianceReport(filePath: string) {
  console.log('='.repeat(60));
  console.log('PCI DSS Compliance Report Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the PCI DSS Compliance Report template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'PCI DSS Compliance Report - SDK Example',
    templateId: 'ptpl_pci_dss_report',
  });
  ...
  // Save results
  const outputCsv = `pci-dss-compliance-report_extracted.csv`;
  const outputJson = `pci-dss-compliance-report_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'PCI DSS Compliance Report - Batch Processing',
    templateId: 'ptpl_pci_dss_report',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
PCI DSS Compliance Report Extraction Example
============================================

Usage:
  npx ts-node pci-dss-compliance-report.ts <document-path>
  npx ts-node pci-dss-compliance-report.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node pci-dss-compliance-report.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/pci-dss-compliance-report
...
(async () => {
  try {
    if (args.length === 1) {
      await extractPciDssComplianceReport(args[0]);
    } else {
