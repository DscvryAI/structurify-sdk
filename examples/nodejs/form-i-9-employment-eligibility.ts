/**
 * Form I-9 (Employment Eligibility) Data Extraction Example
 *
 * Extract structured data from form i-9 (employment eligibility) documents using AI-powered extraction.
 *
 * Extract employee identity, work authorization, and employer verification data from USCIS Form I-9
 *
 * Fields Extracted:
 *   - Form Version
 *   - Last Name
 ...
 * Try it online: https://structurify.ai/extract/form-i-9-employment-eligibility
 *
...
async function extractFormI9EmploymentEligibility(filePath: string) {
  console.log('='.repeat(60));
  console.log('Form I-9 (Employment Eligibility) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Form I-9 (Employment Eligibility) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Form I-9 (Employment Eligibility) - SDK Example',
    templateId: 'ptpl_i9_form',
  });
  ...
  // Save results
  const outputCsv = `form-i-9-employment-eligibility_extracted.csv`;
  const outputJson = `form-i-9-employment-eligibility_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Form I-9 (Employment Eligibility) - Batch Processing',
    templateId: 'ptpl_i9_form',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Form I-9 (Employment Eligibility) Extraction Example
====================================================

Usage:
  npx ts-node form-i-9-employment-eligibility.ts <document-path>
  npx ts-node form-i-9-employment-eligibility.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node form-i-9-employment-eligibility.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/form-i-9-employment-eligibility
...
(async () => {
  try {
    if (args.length === 1) {
      await extractFormI9EmploymentEligibility(args[0]);
    } else {
