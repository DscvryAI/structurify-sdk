/**
 * UK P60 End of Year Certificate Data Extraction Example
 *
 * Extract structured data from uk p60 end of year certificate documents using AI-powered extraction.
 *
 * Extract data from UK P60 tax forms
 *
 * Fields Extracted:
 *   - Tax Year
 *   - Employee Name
 ...
 *
 * Industry: Government
 * Try it online: https://structurify.ai/extract/uk-p60-end-of-year-certificate
 *
...
async function extractUkP60EndOfYearCertificate(filePath: string) {
  console.log('='.repeat(60));
  console.log('UK P60 End of Year Certificate Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the UK P60 End of Year Certificate template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'UK P60 End of Year Certificate - SDK Example',
    templateId: 'ptpl_uk_p60',
  });
  ...
  // Save results
  const outputCsv = `uk-p60-end-of-year-certificate_extracted.csv`;
  const outputJson = `uk-p60-end-of-year-certificate_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'UK P60 End of Year Certificate - Batch Processing',
    templateId: 'ptpl_uk_p60',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
UK P60 End of Year Certificate Extraction Example
=================================================

Usage:
  npx ts-node uk-p60-end-of-year-certificate.ts <document-path>
  npx ts-node uk-p60-end-of-year-certificate.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node uk-p60-end-of-year-certificate.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/uk-p60-end-of-year-certificate
...
(async () => {
  try {
    if (args.length === 1) {
      await extractUkP60EndOfYearCertificate(args[0]);
    } else {
