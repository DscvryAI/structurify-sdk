/**
 * IRS Form 1095-A Data Extraction Example
 *
 * Extract structured data from irs form 1095-a documents using AI-powered extraction.
 *
 * Extract Health Insurance Marketplace Statement data
 *
 * Fields Extracted:
 *   - Marketplace ID
 *   - Policy Number
 ...
 *
 * Industry: Government
 * Try it online: https://structurify.ai/extract/irs-form-1095-a
 *
...
async function extractIrsForm1095A(filePath: string) {
  console.log('='.repeat(60));
  console.log('IRS Form 1095-A Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the IRS Form 1095-A template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'IRS Form 1095-A - SDK Example',
    templateId: 'ptpl_irs_1095a',
  });
  ...
  // Save results
  const outputCsv = `irs-form-1095-a_extracted.csv`;
  const outputJson = `irs-form-1095-a_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'IRS Form 1095-A - Batch Processing',
    templateId: 'ptpl_irs_1095a',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
IRS Form 1095-A Extraction Example
==================================

Usage:
  npx ts-node irs-form-1095-a.ts <document-path>
  npx ts-node irs-form-1095-a.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node irs-form-1095-a.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/irs-form-1095-a
...
(async () => {
  try {
    if (args.length === 1) {
      await extractIrsForm1095A(args[0]);
    } else {
