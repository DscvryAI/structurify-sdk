/**
 * Ontario Laboratory Requisition Data Extraction Example
 *
 * Extract structured data from ontario laboratory requisition documents using AI-powered extraction.
 *
 * Extract data from Ontario lab requisition forms
 *
 * Fields Extracted:
 *   - Patient Name
 *   - Date of Birth
 ...
 *
 * Industry: Healthcare
 * Try it online: https://structurify.ai/extract/ontario-laboratory-requisition
 *
...
async function extractOntarioLaboratoryRequisition(filePath: string) {
  console.log('='.repeat(60));
  console.log('Ontario Laboratory Requisition Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Ontario Laboratory Requisition template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Ontario Laboratory Requisition - SDK Example',
    templateId: 'ptpl_ontario_lab_req',
  });
  ...
  // Save results
  const outputCsv = `ontario-laboratory-requisition_extracted.csv`;
  const outputJson = `ontario-laboratory-requisition_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Ontario Laboratory Requisition - Batch Processing',
    templateId: 'ptpl_ontario_lab_req',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Ontario Laboratory Requisition Extraction Example
=================================================

Usage:
  npx ts-node ontario-laboratory-requisition.ts <document-path>
  npx ts-node ontario-laboratory-requisition.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node ontario-laboratory-requisition.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/ontario-laboratory-requisition
...
(async () => {
  try {
    if (args.length === 1) {
      await extractOntarioLaboratoryRequisition(args[0]);
    } else {
