/**
 * Master Purchase Agreement Data Extraction Example
 *
 * Extract structured data from master purchase agreement documents using AI-powered extraction.
 *
 * Extract key terms from master purchasing agreements and supply agreements
 *
 * Fields Extracted:
 *   - Agreement Title
 *   - Agreement Number
 ...
 * Try it online: https://structurify.ai/extract/master-purchase-agreement
 *
...
async function extractMasterPurchaseAgreement(filePath: string) {
  console.log('='.repeat(60));
  console.log('Master Purchase Agreement Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Master Purchase Agreement template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Master Purchase Agreement - SDK Example',
    templateId: 'ptpl_purchase_agreement',
  });
  ...
  // Save results
  const outputCsv = `master-purchase-agreement_extracted.csv`;
  const outputJson = `master-purchase-agreement_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Master Purchase Agreement - Batch Processing',
    templateId: 'ptpl_purchase_agreement',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Master Purchase Agreement Extraction Example
============================================

Usage:
  npx ts-node master-purchase-agreement.ts <document-path>
  npx ts-node master-purchase-agreement.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node master-purchase-agreement.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/master-purchase-agreement
...
(async () => {
  try {
    if (args.length === 1) {
      await extractMasterPurchaseAgreement(args[0]);
    } else {
