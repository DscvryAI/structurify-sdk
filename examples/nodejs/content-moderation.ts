/**
 * Content Moderation Data Extraction Example
 *
 * Extract structured data from content moderation documents using AI-powered extraction.
 *
 * Analyze images and documents for policy violations, safety concerns, and content classification with severity scoring
 *
 * Fields Extracted:
 *   - Content Type
 ...
 *   - Hate Speech Severity
 *   - Hate Speech Categories
 *
 * Industry: Trust & Safety
 * Try it online: https://structurify.ai/extract/content-moderation
 *
...
async function extractContentModeration(filePath: string) {
  console.log('='.repeat(60));
  console.log('Content Moderation Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Content Moderation template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Content Moderation - SDK Example',
    templateId: 'ptpl_content_moderation',
  });
  ...
  // Save results
  const outputCsv = `content-moderation_extracted.csv`;
  const outputJson = `content-moderation_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Content Moderation - Batch Processing',
    templateId: 'ptpl_content_moderation',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Content Moderation Extraction Example
=====================================

Usage:
  npx ts-node content-moderation.ts <document-path>
  npx ts-node content-moderation.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node content-moderation.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/content-moderation
...
(async () => {
  try {
    if (args.length === 1) {
      await extractContentModeration(args[0]);
    } else {
