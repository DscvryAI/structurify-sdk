/**
 * Certificate of Analysis (COA) Data Extraction Example
 *
 * Extract structured data from certificate of analysis (coa) documents using AI-powered extraction.
 *
 * Extract test results from Certificates of Analysis for incoming material QC verification
 *
 * Fields Extracted:
 *   - Supplier Name
 *   - Supplier Address
 ...
 * Try it online: https://structurify.ai/extract/certificate-of-analysis-coa
 *
...
async function extractCertificateOfAnalysisCoa(filePath: string) {
  console.log('='.repeat(60));
  console.log('Certificate of Analysis (COA) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Certificate of Analysis (COA) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Certificate of Analysis (COA) - SDK Example',
    templateId: 'ptpl_certificate_of_analysis',
  });
  ...
  // Save results
  const outputCsv = `certificate-of-analysis-coa_extracted.csv`;
  const outputJson = `certificate-of-analysis-coa_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Certificate of Analysis (COA) - Batch Processing',
    templateId: 'ptpl_certificate_of_analysis',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Certificate of Analysis (COA) Extraction Example
================================================

Usage:
  npx ts-node certificate-of-analysis-coa.ts <document-path>
  npx ts-node certificate-of-analysis-coa.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node certificate-of-analysis-coa.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/certificate-of-analysis-coa
...
(async () => {
  try {
    if (args.length === 1) {
      await extractCertificateOfAnalysisCoa(args[0]);
    } else {
