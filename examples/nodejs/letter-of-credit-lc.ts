/**
 * Letter of Credit (LC) Data Extraction Example
 *
 * Extract structured data from letter of credit (lc) documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/letter-of-credit-lc
 *
...
async function extractLetterOfCreditLc(filePath: string) {
  console.log('='.repeat(60));
  console.log('Letter of Credit (LC) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Letter of Credit (LC) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Letter of Credit (LC) - SDK Example',
    templateId: 'ptpl_letter_of_credit',
  });
  ...
  // Save results
  const outputCsv = `letter-of-credit-lc_extracted.csv`;
  const outputJson = `letter-of-credit-lc_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Letter of Credit (LC) - Batch Processing',
    templateId: 'ptpl_letter_of_credit',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Letter of Credit (LC) Extraction Example
========================================

Usage:
  npx ts-node letter-of-credit-lc.ts <document-path>
  npx ts-node letter-of-credit-lc.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node letter-of-credit-lc.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/letter-of-credit-lc
...
(async () => {
  try {
    if (args.length === 1) {
      await extractLetterOfCreditLc(args[0]);
    } else {
