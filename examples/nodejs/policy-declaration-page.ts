/**
 * Policy Declaration Page Data Extraction Example
 *
 * Extract structured data from policy declaration page documents using AI-powered extraction.
 *
 * Extract coverage details from insurance policy declaration pages for quoting and comparison
 *
 * Fields Extracted:
 *   - Insured Name
 *   - Insured Address
 ...
 *
 * Industry: Insurance
 * Try it online: https://structurify.ai/extract/policy-declaration-page
 *
...
async function extractPolicyDeclarationPage(filePath: string) {
  console.log('='.repeat(60));
  console.log('Policy Declaration Page Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Policy Declaration Page template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Policy Declaration Page - SDK Example',
    templateId: 'ptpl_policy_dec_page',
  });
  ...
  // Save results
  const outputCsv = `policy-declaration-page_extracted.csv`;
  const outputJson = `policy-declaration-page_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Policy Declaration Page - Batch Processing',
    templateId: 'ptpl_policy_dec_page',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Policy Declaration Page Extraction Example
==========================================

Usage:
  npx ts-node policy-declaration-page.ts <document-path>
  npx ts-node policy-declaration-page.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node policy-declaration-page.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/policy-declaration-page
...
(async () => {
  try {
    if (args.length === 1) {
      await extractPolicyDeclarationPage(args[0]);
    } else {
