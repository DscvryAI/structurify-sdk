/**
 * India GST Invoice Data Extraction Example
 *
 * Extract structured data from india gst invoice documents using AI-powered extraction.
 *
 * Extract data from Indian GST invoices
 *
 * Fields Extracted:
 *   - Supplier Name
 *   - Supplier GSTIN
 ...
 * Try it online: https://structurify.ai/extract/india-gst-invoice
 *
...
async function extractIndiaGstInvoice(filePath: string) {
  console.log('='.repeat(60));
  console.log('India GST Invoice Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the India GST Invoice template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'India GST Invoice - SDK Example',
    templateId: 'ptpl_india_gst_invoice',
  });
  ...
  // Save results
  const outputCsv = `india-gst-invoice_extracted.csv`;
  const outputJson = `india-gst-invoice_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'India GST Invoice - Batch Processing',
    templateId: 'ptpl_india_gst_invoice',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
India GST Invoice Extraction Example
====================================

Usage:
  npx ts-node india-gst-invoice.ts <document-path>
  npx ts-node india-gst-invoice.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node india-gst-invoice.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/india-gst-invoice
...
(async () => {
  try {
    if (args.length === 1) {
      await extractIndiaGstInvoice(args[0]);
    } else {
