/**
 * Expense Reports Data Extraction Example
 *
 * Extract structured data from expense reports documents using AI-powered extraction.
 *
 ...
 *
 * Industry: Financial
 * Try it online: https://structurify.ai/extract/expense-reports
 *
...
async function extractExpenseReports(filePath: string) {
  console.log('='.repeat(60));
  console.log('Expense Reports Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Expense Reports template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Expense Reports - SDK Example',
    templateId: 'ptpl_expense',
  });
  ...
  // Save results
  const outputCsv = `expense-reports_extracted.csv`;
  const outputJson = `expense-reports_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Expense Reports - Batch Processing',
    templateId: 'ptpl_expense',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Expense Reports Extraction Example
==================================

Usage:
  npx ts-node expense-reports.ts <document-path>
  npx ts-node expense-reports.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node expense-reports.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/expense-reports
...
(async () => {
  try {
    if (args.length === 1) {
      await extractExpenseReports(args[0]);
    } else {
