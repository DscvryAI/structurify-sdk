/**
 * Direct Deposit / ACH Authorization Data Extraction Example
 *
 * Extract structured data from direct deposit / ach authorization documents using AI-powered extraction.
 *
 ...
 *
 * Industry: HR / Finance
 * Try it online: https://structurify.ai/extract/direct-deposit-ach-authorization
 *
...
async function extractDirectDepositAchAuthorization(filePath: string) {
  console.log('='.repeat(60));
  console.log('Direct Deposit / ACH Authorization Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Direct Deposit / ACH Authorization template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Direct Deposit / ACH Authorization - SDK Example',
    templateId: 'ptpl_direct_deposit',
  });
  ...
  // Save results
  const outputCsv = `direct-deposit-ach-authorization_extracted.csv`;
  const outputJson = `direct-deposit-ach-authorization_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Direct Deposit / ACH Authorization - Batch Processing',
    templateId: 'ptpl_direct_deposit',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Direct Deposit / ACH Authorization Extraction Example
=====================================================

Usage:
  npx ts-node direct-deposit-ach-authorization.ts <document-path>
  npx ts-node direct-deposit-ach-authorization.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node direct-deposit-ach-authorization.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/direct-deposit-ach-authorization
...
(async () => {
  try {
    if (args.length === 1) {
      await extractDirectDepositAchAuthorization(args[0]);
    } else {
