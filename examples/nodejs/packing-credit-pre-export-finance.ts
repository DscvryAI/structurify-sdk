/**
 * Packing Credit / Pre-Export Finance Data Extraction Example
 *
 * Extract structured data from packing credit / pre-export finance documents using AI-powered extraction.
 *
 * Extract data from packing credit and pre-export financing documents
 *
 * Fields Extracted:
 *   - Facility Reference
 *   - Borrower
 ...
 *   - Tenor
 *
 * Industry: Trade Finance / Banking
 * Try it online: https://structurify.ai/extract/packing-credit-pre-export-finance
 *
...
async function extractPackingCreditPreExportFinance(filePath: string) {
  console.log('='.repeat(60));
  console.log('Packing Credit / Pre-Export Finance Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Packing Credit / Pre-Export Finance template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Packing Credit / Pre-Export Finance - SDK Example',
    templateId: 'ptpl_packing_credit',
  });
  ...
  // Save results
  const outputCsv = `packing-credit-pre-export-finance_extracted.csv`;
  const outputJson = `packing-credit-pre-export-finance_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Packing Credit / Pre-Export Finance - Batch Processing',
    templateId: 'ptpl_packing_credit',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Packing Credit / Pre-Export Finance Extraction Example
======================================================

Usage:
  npx ts-node packing-credit-pre-export-finance.ts <document-path>
  npx ts-node packing-credit-pre-export-finance.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node packing-credit-pre-export-finance.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/packing-credit-pre-export-finance
...
(async () => {
  try {
    if (args.length === 1) {
      await extractPackingCreditPreExportFinance(args[0]);
    } else {
