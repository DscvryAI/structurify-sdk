/**
 * Bill of Lading Data Extraction Example
 *
 * Extract structured data from bill of lading documents using AI-powered extraction.
 *
 * Extract data from ocean/freight bills of lading
 *
 * Fields Extracted:
 *   - B/L Number
 *   - Booking Number
 ...
 * Try it online: https://structurify.ai/extract/bill-of-lading
 *
...
async function extractBillOfLading(filePath: string) {
  console.log('='.repeat(60));
  console.log('Bill of Lading Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Bill of Lading template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Bill of Lading - SDK Example',
    templateId: 'ptpl_bill_of_lading',
  });
  ...
  // Save results
  const outputCsv = `bill-of-lading_extracted.csv`;
  const outputJson = `bill-of-lading_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Bill of Lading - Batch Processing',
    templateId: 'ptpl_bill_of_lading',
  });
  console.log(`Project created: ${project.id}`);
...
Usage:
  npx ts-node bill-of-lading.ts <document-path>
  npx ts-node bill-of-lading.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node bill-of-lading.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/bill-of-lading
...
(async () => {
  try {
    if (args.length === 1) {
      await extractBillOfLading(args[0]);
    } else {
