/**
 * Form 1099-K (Payment Card/Marketplace) Data Extraction Example
 *
 * Extract structured data from form 1099-k (payment card/marketplace) documents using AI-powered extraction.
 *
 * Extract gross payment amounts and transaction data from IRS Form 1099-K for payment cards and third-party networks
 *
 * Fields Extracted:
 *   - Tax Year
 *   - Corrected
 ...
 *   - Payee Address
 *
 * Industry: Tax / E-commerce
 * Try it online: https://structurify.ai/extract/form-1099-k-payment-cardmarketplace
 *
...
async function extractForm1099KPaymentCardmarketplace(filePath: string) {
  console.log('='.repeat(60));
  console.log('Form 1099-K (Payment Card/Marketplace) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Form 1099-K (Payment Card/Marketplace) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Form 1099-K (Payment Card/Marketplace) - SDK Example',
    templateId: 'ptpl_1099k_form',
  });
  ...
  // Save results
  const outputCsv = `form-1099-k-payment-cardmarketplace_extracted.csv`;
  const outputJson = `form-1099-k-payment-cardmarketplace_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Form 1099-K (Payment Card/Marketplace) - Batch Processing',
    templateId: 'ptpl_1099k_form',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Form 1099-K (Payment Card/Marketplace) Extraction Example
=========================================================

Usage:
  npx ts-node form-1099-k-payment-cardmarketplace.ts <document-path>
  npx ts-node form-1099-k-payment-cardmarketplace.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node form-1099-k-payment-cardmarketplace.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/form-1099-k-payment-cardmarketplace
...
(async () => {
  try {
    if (args.length === 1) {
      await extractForm1099KPaymentCardmarketplace(args[0]);
    } else {
