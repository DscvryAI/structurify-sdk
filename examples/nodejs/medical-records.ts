/**
 * Medical Records Data Extraction Example
 *
 * Extract structured data from medical records documents using AI-powered extraction.
 *
 * Extract clinical data from medical documents
 *
 * Fields Extracted:
 *   - Patient ID
 *   - Date of Service
 ...
 *
 * Industry: Healthcare
 * Try it online: https://structurify.ai/extract/medical-records
 *
...
async function extractMedicalRecords(filePath: string) {
  console.log('='.repeat(60));
  console.log('Medical Records Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Medical Records template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Medical Records - SDK Example',
    templateId: 'ptpl_medical_records',
  });
  ...
  // Save results
  const outputCsv = `medical-records_extracted.csv`;
  const outputJson = `medical-records_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Medical Records - Batch Processing',
    templateId: 'ptpl_medical_records',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Medical Records Extraction Example
==================================

Usage:
  npx ts-node medical-records.ts <document-path>
  npx ts-node medical-records.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node medical-records.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/medical-records
...
(async () => {
  try {
    if (args.length === 1) {
      await extractMedicalRecords(args[0]);
    } else {
