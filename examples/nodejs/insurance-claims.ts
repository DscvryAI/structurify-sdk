/**
 * Insurance Claims Data Extraction Example
 *
 * Extract structured data from insurance claims documents using AI-powered extraction.
 *
 * Extract claim data from insurance documents
 *
 * Fields Extracted:
 *   - Claim Number
 *   - Member ID
 ...
 *
 * Industry: Healthcare
 * Try it online: https://structurify.ai/extract/insurance-claims
 *
...
async function extractInsuranceClaims(filePath: string) {
  console.log('='.repeat(60));
  console.log('Insurance Claims Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Insurance Claims template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Insurance Claims - SDK Example',
    templateId: 'ptpl_insurance_claim',
  });
  ...
  // Save results
  const outputCsv = `insurance-claims_extracted.csv`;
  const outputJson = `insurance-claims_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Insurance Claims - Batch Processing',
    templateId: 'ptpl_insurance_claim',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Insurance Claims Extraction Example
===================================

Usage:
  npx ts-node insurance-claims.ts <document-path>
  npx ts-node insurance-claims.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node insurance-claims.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/insurance-claims
...
(async () => {
  try {
    if (args.length === 1) {
      await extractInsuranceClaims(args[0]);
    } else {
