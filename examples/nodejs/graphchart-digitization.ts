/**
 * Graph/Chart Digitization Data Extraction Example
 *
 * Extract structured data from graph/chart digitization documents using AI-powered extraction.
 *
 * Extract X,Y coordinate data points from graphs, charts, and plots
 *
 * Fields Extracted:
 *   - Chart Title
 *   - Chart Type
 ...
 *
 * Industry: Oil & Gas
 * Try it online: https://structurify.ai/extract/graphchart-digitization
 *
...
async function extractGraphchartDigitization(filePath: string) {
  console.log('='.repeat(60));
  console.log('Graph/Chart Digitization Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Graph/Chart Digitization template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Graph/Chart Digitization - SDK Example',
    templateId: 'ptpl_graph_digitization',
  });
  ...
  // Save results
  const outputCsv = `graphchart-digitization_extracted.csv`;
  const outputJson = `graphchart-digitization_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Graph/Chart Digitization - Batch Processing',
    templateId: 'ptpl_graph_digitization',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Graph/Chart Digitization Extraction Example
===========================================

Usage:
  npx ts-node graphchart-digitization.ts <document-path>
  npx ts-node graphchart-digitization.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node graphchart-digitization.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/graphchart-digitization
...
(async () => {
  try {
    if (args.length === 1) {
      await extractGraphchartDigitization(args[0]);
    } else {
