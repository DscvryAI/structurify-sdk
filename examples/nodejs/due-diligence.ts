/**
 * Due Diligence Data Extraction Example
 *
 * Extract structured data from due diligence documents using AI-powered extraction.
 *
 * Extract information for M&A due diligence review
 *
 * Fields Extracted:
 *   - Entity Name
 *   - Jurisdiction
 ...
 *
 * Industry: Legal
 * Try it online: https://structurify.ai/extract/due-diligence
 *
...
async function extractDueDiligence(filePath: string) {
  console.log('='.repeat(60));
  console.log('Due Diligence Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Due Diligence template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Due Diligence - SDK Example',
    templateId: 'ptpl_due_diligence',
  });
  ...
  // Save results
  const outputCsv = `due-diligence_extracted.csv`;
  const outputJson = `due-diligence_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Due Diligence - Batch Processing',
    templateId: 'ptpl_due_diligence',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Due Diligence Extraction Example
================================

Usage:
  npx ts-node due-diligence.ts <document-path>
  npx ts-node due-diligence.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node due-diligence.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/due-diligence
...
(async () => {
  try {
    if (args.length === 1) {
      await extractDueDiligence(args[0]);
    } else {
