/**
 * MSA / SOW / Order Form Data Extraction Example
 *
 * Extract structured data from msa / sow / order form documents using AI-powered extraction.
 *
 * Extract structured terms, deliverables, and pricing from Master Service Agreements, Statements of Work, and Order Forms
 *
 * Fields Extracted:
 *   - Document Type
 *   - Document Title
 ...
 * Try it online: https://structurify.ai/extract/msa-sow-order-form
 *
...
async function extractMsaSowOrderForm(filePath: string) {
  console.log('='.repeat(60));
  console.log('MSA / SOW / Order Form Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the MSA / SOW / Order Form template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'MSA / SOW / Order Form - SDK Example',
    templateId: 'ptpl_msa_sow',
  });
  ...
  // Save results
  const outputCsv = `msa-sow-order-form_extracted.csv`;
  const outputJson = `msa-sow-order-form_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'MSA / SOW / Order Form - Batch Processing',
    templateId: 'ptpl_msa_sow',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
MSA / SOW / Order Form Extraction Example
=========================================

Usage:
  npx ts-node msa-sow-order-form.ts <document-path>
  npx ts-node msa-sow-order-form.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node msa-sow-order-form.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/msa-sow-order-form
...
(async () => {
  try {
    if (args.length === 1) {
      await extractMsaSowOrderForm(args[0]);
    } else {
