/**
 * Invoice Processing Data Extraction Example
 *
 * Extract structured data from invoice processing documents using AI-powered extraction.
 *
 ...
 *
 * Industry: Financial
 * Try it online: https://structurify.ai/extract/invoice-processing
 *
...
async function extractInvoiceProcessing(filePath: string) {
  console.log('='.repeat(60));
  console.log('Invoice Processing Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Invoice Processing template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Invoice Processing - SDK Example',
    templateId: 'ptpl_invoice',
  });
  ...
  // Save results
  const outputCsv = `invoice-processing_extracted.csv`;
  const outputJson = `invoice-processing_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Invoice Processing - Batch Processing',
    templateId: 'ptpl_invoice',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Invoice Processing Extraction Example
=====================================

Usage:
  npx ts-node invoice-processing.ts <document-path>
  npx ts-node invoice-processing.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node invoice-processing.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/invoice-processing
...
(async () => {
  try {
    if (args.length === 1) {
      await extractInvoiceProcessing(args[0]);
    } else {
