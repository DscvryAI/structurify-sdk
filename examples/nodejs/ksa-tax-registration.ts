/**
 * KSA Tax Registration Data Extraction Example
 *
 * Extract structured data from ksa tax registration documents using AI-powered extraction.
 *
 * Extract data from Saudi Arabia tax registration documents
 *
 * Fields Extracted:
 *   - Tax Registration Number
 *   - Taxpayer Name (Arabic)
 ...
 *
 * Industry: Government
 * Try it online: https://structurify.ai/extract/ksa-tax-registration
 *
...
async function extractKsaTaxRegistration(filePath: string) {
  console.log('='.repeat(60));
  console.log('KSA Tax Registration Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the KSA Tax Registration template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'KSA Tax Registration - SDK Example',
    templateId: 'ptpl_ksa_tax_registration',
  });
  ...
  // Save results
  const outputCsv = `ksa-tax-registration_extracted.csv`;
  const outputJson = `ksa-tax-registration_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'KSA Tax Registration - Batch Processing',
    templateId: 'ptpl_ksa_tax_registration',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
KSA Tax Registration Extraction Example
=======================================

Usage:
  npx ts-node ksa-tax-registration.ts <document-path>
  npx ts-node ksa-tax-registration.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node ksa-tax-registration.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/ksa-tax-registration
...
(async () => {
  try {
    if (args.length === 1) {
      await extractKsaTaxRegistration(args[0]);
    } else {
