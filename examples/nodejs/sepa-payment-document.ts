/**
 * SEPA Payment Document Data Extraction Example
 *
 * Extract structured data from sepa payment document documents using AI-powered extraction.
 *
 * Extract data from SEPA credit transfers and direct debits
 *
 * Fields Extracted:
 *   - Transaction Type
 *   - Creditor Name
 ...
 * Try it online: https://structurify.ai/extract/sepa-payment-document
 *
...
async function extractSepaPaymentDocument(filePath: string) {
  console.log('='.repeat(60));
  console.log('SEPA Payment Document Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the SEPA Payment Document template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'SEPA Payment Document - SDK Example',
    templateId: 'ptpl_sepa_payment',
  });
  ...
  // Save results
  const outputCsv = `sepa-payment-document_extracted.csv`;
  const outputJson = `sepa-payment-document_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'SEPA Payment Document - Batch Processing',
    templateId: 'ptpl_sepa_payment',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
SEPA Payment Document Extraction Example
========================================

Usage:
  npx ts-node sepa-payment-document.ts <document-path>
  npx ts-node sepa-payment-document.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node sepa-payment-document.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/sepa-payment-document
...
(async () => {
  try {
    if (args.length === 1) {
      await extractSepaPaymentDocument(args[0]);
    } else {
