/**
 * Healthcare EDI Documents Data Extraction Example
 *
 * Extract structured data from healthcare edi documents documents using AI-powered extraction.
 *
 * Extract data from EDI 835/837 healthcare transactions
 *
 * Fields Extracted:
 *   - Transaction Type
 *   - Sender ID
 ...
 *
 * Industry: Healthcare
 * Try it online: https://structurify.ai/extract/healthcare-edi-documents
 *
...
async function extractHealthcareEdiDocuments(filePath: string) {
  console.log('='.repeat(60));
  console.log('Healthcare EDI Documents Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Healthcare EDI Documents template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Healthcare EDI Documents - SDK Example',
    templateId: 'ptpl_edi_healthcare',
  });
  ...
  // Save results
  const outputCsv = `healthcare-edi-documents_extracted.csv`;
  const outputJson = `healthcare-edi-documents_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Healthcare EDI Documents - Batch Processing',
    templateId: 'ptpl_edi_healthcare',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Healthcare EDI Documents Extraction Example
===========================================

Usage:
  npx ts-node healthcare-edi-documents.ts <document-path>
  npx ts-node healthcare-edi-documents.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node healthcare-edi-documents.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/healthcare-edi-documents
...
(async () => {
  try {
    if (args.length === 1) {
      await extractHealthcareEdiDocuments(args[0]);
    } else {
