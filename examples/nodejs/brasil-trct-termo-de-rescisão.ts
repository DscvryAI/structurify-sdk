/**
 * Brasil - TRCT (Termo de Rescisão) Data Extraction Example
 *
 * Extract structured data from brasil - trct (termo de rescisão) documents using AI-powered extraction.
 *
 * Extrair dados do Termo de Rescisão do Contrato de Trabalho para auditoria de verbas rescisórias
 *
 * Fields Extracted:
 *   - CNPJ Empregador
 *   - Razão Social
 ...
 *
 * Industry: HR / Legal
 * Try it online: https://structurify.ai/extract/brasil-trct-termo-de-rescisão
 *
...
async function extractBrasilTrctTermoDeRescisão(filePath: string) {
  console.log('='.repeat(60));
  console.log('Brasil - TRCT (Termo de Rescisão) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Brasil - TRCT (Termo de Rescisão) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Brasil - TRCT (Termo de Rescisão) - SDK Example',
    templateId: 'ptpl_brazil_trct',
  });
  ...
  // Save results
  const outputCsv = `brasil-trct-termo-de-rescisão_extracted.csv`;
  const outputJson = `brasil-trct-termo-de-rescisão_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Brasil - TRCT (Termo de Rescisão) - Batch Processing',
    templateId: 'ptpl_brazil_trct',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Brasil - TRCT (Termo de Rescisão) Extraction Example
====================================================

Usage:
  npx ts-node brasil-trct-termo-de-rescisão.ts <document-path>
  npx ts-node brasil-trct-termo-de-rescisão.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node brasil-trct-termo-de-rescisão.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/brasil-trct-termo-de-rescisão
...
(async () => {
  try {
    if (args.length === 1) {
      await extractBrasilTrctTermoDeRescisão(args[0]);
    } else {
