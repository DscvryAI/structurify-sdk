/**
 * Lien Waiver Data Extraction Example
 *
 * Extract structured data from lien waiver documents using AI-powered extraction.
 *
 * Extract data from conditional and unconditional lien waivers (progress and final)
 *
 * Fields Extracted:
 *   - Waiver Type
 *   - Claimant Name
 ...
 * Try it online: https://structurify.ai/extract/lien-waiver
 *
...
async function extractLienWaiver(filePath: string) {
  console.log('='.repeat(60));
  console.log('Lien Waiver Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Lien Waiver template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Lien Waiver - SDK Example',
    templateId: 'ptpl_lien_waiver',
  });
  ...
  // Save results
  const outputCsv = `lien-waiver_extracted.csv`;
  const outputJson = `lien-waiver_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Lien Waiver - Batch Processing',
    templateId: 'ptpl_lien_waiver',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Lien Waiver Extraction Example
==============================

Usage:
  npx ts-node lien-waiver.ts <document-path>
  npx ts-node lien-waiver.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node lien-waiver.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/lien-waiver
...
(async () => {
  try {
    if (args.length === 1) {
      await extractLienWaiver(args[0]);
    } else {
