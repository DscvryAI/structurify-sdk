/**
 * Vendor Performance Scorecard Data Extraction Example
 *
 * Extract structured data from vendor performance scorecard documents using AI-powered extraction.
 *
 * Extract data from vendor/supplier performance evaluations
 *
 * Fields Extracted:
 *   - Vendor Name
 *   - Vendor Code
 ...
 * Try it online: https://structurify.ai/extract/vendor-performance-scorecard
 *
...
async function extractVendorPerformanceScorecard(filePath: string) {
  console.log('='.repeat(60));
  console.log('Vendor Performance Scorecard Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Vendor Performance Scorecard template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Vendor Performance Scorecard - SDK Example',
    templateId: 'ptpl_vendor_scorecard',
  });
  ...
  // Save results
  const outputCsv = `vendor-performance-scorecard_extracted.csv`;
  const outputJson = `vendor-performance-scorecard_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Vendor Performance Scorecard - Batch Processing',
    templateId: 'ptpl_vendor_scorecard',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Vendor Performance Scorecard Extraction Example
===============================================

Usage:
  npx ts-node vendor-performance-scorecard.ts <document-path>
  npx ts-node vendor-performance-scorecard.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node vendor-performance-scorecard.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/vendor-performance-scorecard
...
(async () => {
  try {
    if (args.length === 1) {
      await extractVendorPerformanceScorecard(args[0]);
    } else {
