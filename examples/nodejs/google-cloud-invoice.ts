/**
 * Google Cloud Invoice Data Extraction Example
 *
 * Extract structured data from google cloud invoice documents using AI-powered extraction.
 *
 * Extract billing details, service charges, and usage data from Google Cloud Platform invoices and billing statements
 *
 * Fields Extracted:
 *   - Invoice Number
 *   - Invoice Date
 ...
 * Try it online: https://structurify.ai/extract/google-cloud-invoice
 *
...
async function extractGoogleCloudInvoice(filePath: string) {
  console.log('='.repeat(60));
  console.log('Google Cloud Invoice Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Google Cloud Invoice template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Google Cloud Invoice - SDK Example',
    templateId: 'ptpl_gcp_invoice',
  });
  ...
  // Save results
  const outputCsv = `google-cloud-invoice_extracted.csv`;
  const outputJson = `google-cloud-invoice_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Google Cloud Invoice - Batch Processing',
    templateId: 'ptpl_gcp_invoice',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Google Cloud Invoice Extraction Example
=======================================

Usage:
  npx ts-node google-cloud-invoice.ts <document-path>
  npx ts-node google-cloud-invoice.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node google-cloud-invoice.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/google-cloud-invoice
...
(async () => {
  try {
    if (args.length === 1) {
      await extractGoogleCloudInvoice(args[0]);
    } else {
