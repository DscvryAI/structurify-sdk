/**
 * Property Inspection Data Extraction Example
 *
 * Extract structured data from property inspection documents using AI-powered extraction.
 *
 * Extract findings from property inspection reports
 *
 * Fields Extracted:
 *   - Property Address
 *   - Inspection Date
 ...
 * Try it online: https://structurify.ai/extract/property-inspection
 *
...
async function extractPropertyInspection(filePath: string) {
  console.log('='.repeat(60));
  console.log('Property Inspection Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Property Inspection template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Property Inspection - SDK Example',
    templateId: 'ptpl_property_inspection',
  });
  ...
  // Save results
  const outputCsv = `property-inspection_extracted.csv`;
  const outputJson = `property-inspection_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Property Inspection - Batch Processing',
    templateId: 'ptpl_property_inspection',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Property Inspection Extraction Example
======================================

Usage:
  npx ts-node property-inspection.ts <document-path>
  npx ts-node property-inspection.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node property-inspection.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/property-inspection
...
(async () => {
  try {
    if (args.length === 1) {
      await extractPropertyInspection(args[0]);
    } else {
