/**
 * India Electricity Bill Data Extraction Example
 *
 * Extract structured data from india electricity bill documents using AI-powered extraction.
 *
 * Extract data from Indian electricity bills
 *
 * Fields Extracted:
 *   - Utility Company
 *   - Consumer Number
 ...
 * Try it online: https://structurify.ai/extract/india-electricity-bill
 *
...
async function extractIndiaElectricityBill(filePath: string) {
  console.log('='.repeat(60));
  console.log('India Electricity Bill Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the India Electricity Bill template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'India Electricity Bill - SDK Example',
    templateId: 'ptpl_india_electricity_bill',
  });
  ...
  // Save results
  const outputCsv = `india-electricity-bill_extracted.csv`;
  const outputJson = `india-electricity-bill_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'India Electricity Bill - Batch Processing',
    templateId: 'ptpl_india_electricity_bill',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
India Electricity Bill Extraction Example
=========================================

Usage:
  npx ts-node india-electricity-bill.ts <document-path>
  npx ts-node india-electricity-bill.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node india-electricity-bill.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/india-electricity-bill
...
(async () => {
  try {
    if (args.length === 1) {
      await extractIndiaElectricityBill(args[0]);
    } else {
