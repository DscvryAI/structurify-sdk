/**
 * FDA 483 Observations Data Extraction Example
 *
 * Extract structured data from fda 483 observations documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/fda-483-observations
 *
...
async function extractFda483Observations(filePath: string) {
  console.log('='.repeat(60));
  console.log('FDA 483 Observations Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the FDA 483 Observations template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'FDA 483 Observations - SDK Example',
    templateId: 'ptpl_fda_483',
  });
  ...
  // Save results
  const outputCsv = `fda-483-observations_extracted.csv`;
  const outputJson = `fda-483-observations_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'FDA 483 Observations - Batch Processing',
    templateId: 'ptpl_fda_483',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
FDA 483 Observations Extraction Example
=======================================

Usage:
  npx ts-node fda-483-observations.ts <document-path>
  npx ts-node fda-483-observations.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node fda-483-observations.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/fda-483-observations
...
(async () => {
  try {
    if (args.length === 1) {
      await extractFda483Observations(args[0]);
    } else {
