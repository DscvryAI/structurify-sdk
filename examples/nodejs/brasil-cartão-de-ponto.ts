/**
 * Brasil - Cartão de Ponto Data Extraction Example
 *
 * Extract structured data from brasil - cartão de ponto documents using AI-powered extraction.
 *
 * Extrair dados de registro de ponto para cálculo de horas extras e auditoria trabalhista
 *
 * Fields Extracted:
 *   - Nome Empregado
 *   - Matrícula
 ...
 *
 * Industry: HR / Legal
 * Try it online: https://structurify.ai/extract/brasil-cartão-de-ponto
 *
...
async function extractBrasilCartãoDePonto(filePath: string) {
  console.log('='.repeat(60));
  console.log('Brasil - Cartão de Ponto Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Brasil - Cartão de Ponto template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Brasil - Cartão de Ponto - SDK Example',
    templateId: 'ptpl_brazil_timecard',
  });
  ...
  // Save results
  const outputCsv = `brasil-cartão-de-ponto_extracted.csv`;
  const outputJson = `brasil-cartão-de-ponto_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Brasil - Cartão de Ponto - Batch Processing',
    templateId: 'ptpl_brazil_timecard',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Brasil - Cartão de Ponto Extraction Example
===========================================

Usage:
  npx ts-node brasil-cartão-de-ponto.ts <document-path>
  npx ts-node brasil-cartão-de-ponto.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node brasil-cartão-de-ponto.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/brasil-cartão-de-ponto
...
(async () => {
  try {
    if (args.length === 1) {
      await extractBrasilCartãoDePonto(args[0]);
    } else {
