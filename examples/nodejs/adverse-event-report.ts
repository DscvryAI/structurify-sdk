/**
 * Adverse Event Report Data Extraction Example
 *
 * Extract structured data from adverse event report documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/adverse-event-report
 *
...
async function extractAdverseEventReport(filePath: string) {
  console.log('='.repeat(60));
  console.log('Adverse Event Report Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Adverse Event Report template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Adverse Event Report - SDK Example',
    templateId: 'ptpl_adverse_event_report',
  });
  ...
  // Save results
  const outputCsv = `adverse-event-report_extracted.csv`;
  const outputJson = `adverse-event-report_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Adverse Event Report - Batch Processing',
    templateId: 'ptpl_adverse_event_report',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Adverse Event Report Extraction Example
=======================================

Usage:
  npx ts-node adverse-event-report.ts <document-path>
  npx ts-node adverse-event-report.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node adverse-event-report.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/adverse-event-report
...
(async () => {
  try {
    if (args.length === 1) {
      await extractAdverseEventReport(args[0]);
    } else {
