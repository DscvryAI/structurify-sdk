/**
 * CMR International Consignment Note Data Extraction Example
 *
 * Extract structured data from cmr international consignment note documents using AI-powered extraction.
 *
 * Extract data from CMR international road transport documents
 *
 * Fields Extracted:
 *   - CMR Number
 *   - Date of Issue
 ...
 *
 * Industry: Legal
 * Try it online: https://structurify.ai/extract/cmr-international-consignment-note
 *
...
async function extractCmrInternationalConsignmentNote(filePath: string) {
  console.log('='.repeat(60));
  console.log('CMR International Consignment Note Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the CMR International Consignment Note template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'CMR International Consignment Note - SDK Example',
    templateId: 'ptpl_cmr_consignment',
  });
  ...
  // Save results
  const outputCsv = `cmr-international-consignment-note_extracted.csv`;
  const outputJson = `cmr-international-consignment-note_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'CMR International Consignment Note - Batch Processing',
    templateId: 'ptpl_cmr_consignment',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
CMR International Consignment Note Extraction Example
=====================================================

Usage:
  npx ts-node cmr-international-consignment-note.ts <document-path>
  npx ts-node cmr-international-consignment-note.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node cmr-international-consignment-note.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/cmr-international-consignment-note
...
(async () => {
  try {
    if (args.length === 1) {
      await extractCmrInternationalConsignmentNote(args[0]);
    } else {
