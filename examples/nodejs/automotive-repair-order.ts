/**
 * Automotive Repair Order Data Extraction Example
 *
 * Extract structured data from automotive repair order documents using AI-powered extraction.
 *
 * Extract vehicle data, labor, and line-item parts from automotive service invoices and repair orders
 *
 * Fields Extracted:
 *   - Repair Order Number
 *   - Invoice Date
 ...
 *   - Unit Number
 *
 * Industry: Fleet Management / Automotive
 * Try it online: https://structurify.ai/extract/automotive-repair-order
 *
...
async function extractAutomotiveRepairOrder(filePath: string) {
  console.log('='.repeat(60));
  console.log('Automotive Repair Order Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Automotive Repair Order template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Automotive Repair Order - SDK Example',
    templateId: 'ptpl_auto_repair_order',
  });
  ...
  // Save results
  const outputCsv = `automotive-repair-order_extracted.csv`;
  const outputJson = `automotive-repair-order_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Automotive Repair Order - Batch Processing',
    templateId: 'ptpl_auto_repair_order',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Automotive Repair Order Extraction Example
==========================================

Usage:
  npx ts-node automotive-repair-order.ts <document-path>
  npx ts-node automotive-repair-order.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node automotive-repair-order.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/automotive-repair-order
...
(async () => {
  try {
    if (args.length === 1) {
      await extractAutomotiveRepairOrder(args[0]);
    } else {
