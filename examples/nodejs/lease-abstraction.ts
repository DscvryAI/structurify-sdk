/**
 * Lease Abstraction Data Extraction Example
 *
 * Extract structured data from lease abstraction documents using AI-powered extraction.
 *
 * Extract key terms from commercial leases
 *
 * Fields Extracted:
 *   - Property Address
 *   - Tenant
 ...
 * Try it online: https://structurify.ai/extract/lease-abstraction
 *
...
async function extractLeaseAbstraction(filePath: string) {
  console.log('='.repeat(60));
  console.log('Lease Abstraction Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Lease Abstraction template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Lease Abstraction - SDK Example',
    templateId: 'ptpl_lease',
  });
  ...
  // Save results
  const outputCsv = `lease-abstraction_extracted.csv`;
  const outputJson = `lease-abstraction_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Lease Abstraction - Batch Processing',
    templateId: 'ptpl_lease',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Lease Abstraction Extraction Example
====================================

Usage:
  npx ts-node lease-abstraction.ts <document-path>
  npx ts-node lease-abstraction.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node lease-abstraction.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/lease-abstraction
...
(async () => {
  try {
    if (args.length === 1) {
      await extractLeaseAbstraction(args[0]);
    } else {
