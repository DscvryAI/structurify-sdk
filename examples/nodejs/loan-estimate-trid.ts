/**
 * Loan Estimate (TRID) Data Extraction Example
 *
 * Extract structured data from loan estimate (trid) documents using AI-powered extraction.
 *
 * Extract loan terms, fees, and disclosures from CFPB Loan Estimate forms for mortgage comparison
 *
 * Fields Extracted:
 *   - Date Issued
 *   - Applicant Name
 ...
 * Try it online: https://structurify.ai/extract/loan-estimate-trid
 *
...
async function extractLoanEstimateTrid(filePath: string) {
  console.log('='.repeat(60));
  console.log('Loan Estimate (TRID) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Loan Estimate (TRID) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Loan Estimate (TRID) - SDK Example',
    templateId: 'ptpl_loan_estimate',
  });
  ...
  // Save results
  const outputCsv = `loan-estimate-trid_extracted.csv`;
  const outputJson = `loan-estimate-trid_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Loan Estimate (TRID) - Batch Processing',
    templateId: 'ptpl_loan_estimate',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Loan Estimate (TRID) Extraction Example
=======================================

Usage:
  npx ts-node loan-estimate-trid.ts <document-path>
  npx ts-node loan-estimate-trid.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node loan-estimate-trid.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/loan-estimate-trid
...
(async () => {
  try {
    if (args.length === 1) {
      await extractLoanEstimateTrid(args[0]);
    } else {
