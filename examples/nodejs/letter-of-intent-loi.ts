/**
 * Letter of Intent (LOI) Data Extraction Example
 *
 * Extract structured data from letter of intent (loi) documents using AI-powered extraction.
 *
 * Extract key terms from M&A letters of intent and indication of interest
 *
 * Fields Extracted:
 *   - Transaction Type
 *   - Buyer/Acquirer
 ...
 *
 * Industry: M&A / Corporate
 * Try it online: https://structurify.ai/extract/letter-of-intent-loi
 *
...
async function extractLetterOfIntentLoi(filePath: string) {
  console.log('='.repeat(60));
  console.log('Letter of Intent (LOI) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Letter of Intent (LOI) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Letter of Intent (LOI) - SDK Example',
    templateId: 'ptpl_letter_of_intent',
  });
  ...
  // Save results
  const outputCsv = `letter-of-intent-loi_extracted.csv`;
  const outputJson = `letter-of-intent-loi_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Letter of Intent (LOI) - Batch Processing',
    templateId: 'ptpl_letter_of_intent',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Letter of Intent (LOI) Extraction Example
=========================================

Usage:
  npx ts-node letter-of-intent-loi.ts <document-path>
  npx ts-node letter-of-intent-loi.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node letter-of-intent-loi.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/letter-of-intent-loi
...
(async () => {
  try {
    if (args.length === 1) {
      await extractLetterOfIntentLoi(args[0]);
    } else {
