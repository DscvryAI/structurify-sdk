/**
 * Brasil - Holerite / Contracheque Data Extraction Example
 *
 * Extract structured data from brasil - holerite / contracheque documents using AI-powered extraction.
 *
 * Extrair dados do holerite/contracheque brasileiro para auditoria de pagamentos
 *
 * Fields Extracted:
 *   - Empresa
 *   - CNPJ
 ...
 *
 * Industry: HR / Legal
 * Try it online: https://structurify.ai/extract/brasil-holerite-contracheque
 *
...
async function extractBrasilHoleriteContracheque(filePath: string) {
  console.log('='.repeat(60));
  console.log('Brasil - Holerite / Contracheque Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Brasil - Holerite / Contracheque template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Brasil - Holerite / Contracheque - SDK Example',
    templateId: 'ptpl_brazil_payslip',
  });
  ...
  // Save results
  const outputCsv = `brasil-holerite-contracheque_extracted.csv`;
  const outputJson = `brasil-holerite-contracheque_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Brasil - Holerite / Contracheque - Batch Processing',
    templateId: 'ptpl_brazil_payslip',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Brasil - Holerite / Contracheque Extraction Example
===================================================

Usage:
  npx ts-node brasil-holerite-contracheque.ts <document-path>
  npx ts-node brasil-holerite-contracheque.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node brasil-holerite-contracheque.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/brasil-holerite-contracheque
...
(async () => {
  try {
    if (args.length === 1) {
      await extractBrasilHoleriteContracheque(args[0]);
    } else {
