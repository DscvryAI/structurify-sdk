/**
 * Medical Referral Letter Data Extraction Example
 *
 * Extract structured data from medical referral letter documents using AI-powered extraction.
 *
 * Extract referral details from physician referral letters for prior authorization and scheduling
 *
 * Fields Extracted:
 *   - Referring Provider
 ...
 * Try it online: https://structurify.ai/extract/medical-referral-letter
 *
...
async function extractMedicalReferralLetter(filePath: string) {
  console.log('='.repeat(60));
  console.log('Medical Referral Letter Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Medical Referral Letter template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Medical Referral Letter - SDK Example',
    templateId: 'ptpl_referral_letter',
  });
  ...
  // Save results
  const outputCsv = `medical-referral-letter_extracted.csv`;
  const outputJson = `medical-referral-letter_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Medical Referral Letter - Batch Processing',
    templateId: 'ptpl_referral_letter',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Medical Referral Letter Extraction Example
==========================================

Usage:
  npx ts-node medical-referral-letter.ts <document-path>
  npx ts-node medical-referral-letter.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node medical-referral-letter.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/medical-referral-letter
...
(async () => {
  try {
    if (args.length === 1) {
      await extractMedicalReferralLetter(args[0]);
    } else {
