/**
 * KYC - Individual Client Data Extraction Example
 *
 * Extract structured data from kyc - individual client documents using AI-powered extraction.
 *
 * Extract data from Know Your Customer documents for individual clients
 *
 * Fields Extracted:
 *   - Full Legal Name
 *   - Date of Birth
 ...
 *   - Mailing Address
 *
 * Industry: Wealth Management / Compliance
 * Try it online: https://structurify.ai/extract/kyc-individual-client
 *
...
async function extractKycIndividualClient(filePath: string) {
  console.log('='.repeat(60));
  console.log('KYC - Individual Client Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the KYC - Individual Client template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'KYC - Individual Client - SDK Example',
    templateId: 'ptpl_kyc_individual',
  });
  ...
  // Save results
  const outputCsv = `kyc-individual-client_extracted.csv`;
  const outputJson = `kyc-individual-client_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'KYC - Individual Client - Batch Processing',
    templateId: 'ptpl_kyc_individual',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
KYC - Individual Client Extraction Example
==========================================

Usage:
  npx ts-node kyc-individual-client.ts <document-path>
  npx ts-node kyc-individual-client.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node kyc-individual-client.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/kyc-individual-client
...
(async () => {
  try {
    if (args.length === 1) {
      await extractKycIndividualClient(args[0]);
    } else {
