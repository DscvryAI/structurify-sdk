/**
 * Biostratigraphy Report Data Extraction Example
 *
 * Extract structured data from biostratigraphy report documents using AI-powered extraction.
 *
 * Extract fossil events, zonation, and age data from biostratigraphy/nannofossil reports
 *
 * Fields Extracted:
 *   - Well/Location Name
 *   - Report Date
 ...
 *   - Age Range
 *   - Depth Range
 *   - Chronostratigraphic Summary
 *   - Biozone Summary
 *
 * Industry: Oil & Gas
 * Try it online: https://structurify.ai/extract/biostratigraphy-report
 *
...
async function extractBiostratigraphyReport(filePath: string) {
  console.log('='.repeat(60));
  console.log('Biostratigraphy Report Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Biostratigraphy Report template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Biostratigraphy Report - SDK Example',
    templateId: 'ptpl_biostratigraphy_report',
  });
  ...
  // Save results
  const outputCsv = `biostratigraphy-report_extracted.csv`;
  const outputJson = `biostratigraphy-report_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Biostratigraphy Report - Batch Processing',
    templateId: 'ptpl_biostratigraphy_report',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Biostratigraphy Report Extraction Example
=========================================

Usage:
  npx ts-node biostratigraphy-report.ts <document-path>
  npx ts-node biostratigraphy-report.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node biostratigraphy-report.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/biostratigraphy-report
...
(async () => {
  try {
    if (args.length === 1) {
      await extractBiostratigraphyReport(args[0]);
    } else {
