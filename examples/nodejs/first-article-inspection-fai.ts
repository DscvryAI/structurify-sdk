/**
 * First Article Inspection (FAI) Data Extraction Example
 *
 * Extract structured data from first article inspection (fai) documents using AI-powered extraction.
 *
 * Extract data from AS9102 First Article Inspection Reports
 *
 * Fields Extracted:
 *   - FAI Report Number
 *   - Part Number
 ...
 * Try it online: https://structurify.ai/extract/first-article-inspection-fai
 *
...
async function extractFirstArticleInspectionFai(filePath: string) {
  console.log('='.repeat(60));
  console.log('First Article Inspection (FAI) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the First Article Inspection (FAI) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'First Article Inspection (FAI) - SDK Example',
    templateId: 'ptpl_first_article_inspection',
  });
  ...
  // Save results
  const outputCsv = `first-article-inspection-fai_extracted.csv`;
  const outputJson = `first-article-inspection-fai_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'First Article Inspection (FAI) - Batch Processing',
    templateId: 'ptpl_first_article_inspection',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
First Article Inspection (FAI) Extraction Example
=================================================

Usage:
  npx ts-node first-article-inspection-fai.ts <document-path>
  npx ts-node first-article-inspection-fai.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node first-article-inspection-fai.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/first-article-inspection-fai
...
(async () => {
  try {
    if (args.length === 1) {
      await extractFirstArticleInspectionFai(args[0]);
    } else {
