/**
 * US Driver's License Data Extraction Example
 *
 * Extract structured data from us driver's license documents using AI-powered extraction.
 *
 * Extract data from US driver's licenses
 *
 * Fields Extracted:
 *   - State
 *   - License Number
 ...
 * Try it online: https://structurify.ai/extract/us-drivers-license
 *
...
async function extractUsDriversLicense(filePath: string) {
  console.log('='.repeat(60));
  console.log('US Driver's License Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the US Driver's License template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'US Driver's License - SDK Example',
    templateId: 'ptpl_drivers_license_us',
  });
  ...
  // Save results
  const outputCsv = `us-drivers-license_extracted.csv`;
  const outputJson = `us-drivers-license_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'US Driver's License - Batch Processing',
    templateId: 'ptpl_drivers_license_us',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
US Driver's License Extraction Example
======================================

Usage:
  npx ts-node us-drivers-license.ts <document-path>
  npx ts-node us-drivers-license.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node us-drivers-license.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/us-drivers-license
...
(async () => {
  try {
    if (args.length === 1) {
      await extractUsDriversLicense(args[0]);
    } else {
