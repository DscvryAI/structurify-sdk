/**
 * Packing List Data Extraction Example
 *
 * Extract structured data from packing list documents using AI-powered extraction.
 *
 * Extract data from commercial packing lists
 *
 * Fields Extracted:
 *   - Packing List Number
 *   - Invoice Number
 ...
 * Try it online: https://structurify.ai/extract/packing-list
 *
...
async function extractPackingList(filePath: string) {
  console.log('='.repeat(60));
  console.log('Packing List Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Packing List template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Packing List - SDK Example',
    templateId: 'ptpl_packing_list',
  });
  ...
  // Save results
  const outputCsv = `packing-list_extracted.csv`;
  const outputJson = `packing-list_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Packing List - Batch Processing',
    templateId: 'ptpl_packing_list',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Packing List Extraction Example
===============================

Usage:
  npx ts-node packing-list.ts <document-path>
  npx ts-node packing-list.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node packing-list.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/packing-list
...
(async () => {
  try {
    if (args.length === 1) {
      await extractPackingList(args[0]);
    } else {
