/**
 * Punch List Data Extraction Example
 *
 * Extract structured data from punch list documents using AI-powered extraction.
 *
 * Extract data from construction punch lists
 *
 * Fields Extracted:
 *   - Project Name
 *   - Project Number
 ...
 * Try it online: https://structurify.ai/extract/punch-list
 *
...
async function extractPunchList(filePath: string) {
  console.log('='.repeat(60));
  console.log('Punch List Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Punch List template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Punch List - SDK Example',
    templateId: 'ptpl_punch_list',
  });
  ...
  // Save results
  const outputCsv = `punch-list_extracted.csv`;
  const outputJson = `punch-list_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Punch List - Batch Processing',
    templateId: 'ptpl_punch_list',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Punch List Extraction Example
=============================

Usage:
  npx ts-node punch-list.ts <document-path>
  npx ts-node punch-list.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node punch-list.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/punch-list
