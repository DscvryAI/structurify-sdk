/**
 * GDPR Data Subject Access Request Data Extraction Example
 *
 * Extract structured data from gdpr data subject access request documents using AI-powered extraction.
 *
 * Extract data from GDPR DSAR documents
 *
 * Fields Extracted:
 *   - Request Date
 *   - Requester Name
 ...
 *
 * Industry: Legal
 * Try it online: https://structurify.ai/extract/gdpr-data-subject-access-request
 *
...
async function extractGdprDataSubjectAccessRequest(filePath: string) {
  console.log('='.repeat(60));
  console.log('GDPR Data Subject Access Request Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the GDPR Data Subject Access Request template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'GDPR Data Subject Access Request - SDK Example',
    templateId: 'ptpl_gdpr_dsar',
  });
  ...
  // Save results
  const outputCsv = `gdpr-data-subject-access-request_extracted.csv`;
  const outputJson = `gdpr-data-subject-access-request_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'GDPR Data Subject Access Request - Batch Processing',
    templateId: 'ptpl_gdpr_dsar',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
GDPR Data Subject Access Request Extraction Example
===================================================

Usage:
  npx ts-node gdpr-data-subject-access-request.ts <document-path>
  npx ts-node gdpr-data-subject-access-request.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node gdpr-data-subject-access-request.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/gdpr-data-subject-access-request
...
(async () => {
  try {
    if (args.length === 1) {
      await extractGdprDataSubjectAccessRequest(args[0]);
    } else {
