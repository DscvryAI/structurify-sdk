/**
 * Privacy Policy Analysis Data Extraction Example
 *
 * Extract structured data from privacy policy analysis documents using AI-powered extraction.
 *
 * Analyze privacy policies for compliance, data practices, and consumer rights disclosures
 *
 * Fields Extracted:
 *   - Company Name
 ...
 * Try it online: https://structurify.ai/extract/privacy-policy-analysis
 *
...
async function extractPrivacyPolicyAnalysis(filePath: string) {
  console.log('='.repeat(60));
  console.log('Privacy Policy Analysis Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Privacy Policy Analysis template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Privacy Policy Analysis - SDK Example',
    templateId: 'ptpl_privacy_policy',
  });
  ...
  // Save results
  const outputCsv = `privacy-policy-analysis_extracted.csv`;
  const outputJson = `privacy-policy-analysis_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Privacy Policy Analysis - Batch Processing',
    templateId: 'ptpl_privacy_policy',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Privacy Policy Analysis Extraction Example
==========================================

Usage:
  npx ts-node privacy-policy-analysis.ts <document-path>
  npx ts-node privacy-policy-analysis.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node privacy-policy-analysis.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/privacy-policy-analysis
...
(async () => {
  try {
    if (args.length === 1) {
      await extractPrivacyPolicyAnalysis(args[0]);
    } else {
