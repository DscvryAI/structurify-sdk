/**
 * Form W-8BEN (Individual) Data Extraction Example
 *
 * Extract structured data from form w-8ben (individual) documents using AI-powered extraction.
 *
 * Extract foreign status and treaty benefits from IRS Form W-8BEN for individual foreign persons
 *
 * Fields Extracted:
 *   - Form Revision
 *   - Name
 ...
 *   - US TIN
 *   - Foreign TIN
 *   - FTIN Not Required Reason
 *   - Reference Numbers
 *   - Date of Birth
 *
 * Industry: Tax / Compliance
 * Try it online: https://structurify.ai/extract/form-w-8ben-individual
 *
...
async function extractFormW8BenIndividual(filePath: string) {
  console.log('='.repeat(60));
  console.log('Form W-8BEN (Individual) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Form W-8BEN (Individual) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Form W-8BEN (Individual) - SDK Example',
    templateId: 'ptpl_w8ben',
  });
  ...
  // Save results
  const outputCsv = `form-w-8ben-individual_extracted.csv`;
  const outputJson = `form-w-8ben-individual_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Form W-8BEN (Individual) - Batch Processing',
    templateId: 'ptpl_w8ben',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Form W-8BEN (Individual) Extraction Example
===========================================

Usage:
  npx ts-node form-w-8ben-individual.ts <document-path>
  npx ts-node form-w-8ben-individual.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node form-w-8ben-individual.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/form-w-8ben-individual
...
(async () => {
  try {
    if (args.length === 1) {
      await extractFormW8BenIndividual(args[0]);
    } else {
