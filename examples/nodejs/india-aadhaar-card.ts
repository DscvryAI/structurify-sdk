/**
 * India Aadhaar Card Data Extraction Example
 *
 * Extract structured data from india aadhaar card documents using AI-powered extraction.
 *
 * Extract data from Indian Aadhaar cards
 *
 * Fields Extracted:
 *   - Aadhaar Number
 *   - Name
 ...
 * Try it online: https://structurify.ai/extract/india-aadhaar-card
 *
...
async function extractIndiaAadhaarCard(filePath: string) {
  console.log('='.repeat(60));
  console.log('India Aadhaar Card Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the India Aadhaar Card template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'India Aadhaar Card - SDK Example',
    templateId: 'ptpl_national_id_india',
  });
  ...
  // Save results
  const outputCsv = `india-aadhaar-card_extracted.csv`;
  const outputJson = `india-aadhaar-card_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'India Aadhaar Card - Batch Processing',
    templateId: 'ptpl_national_id_india',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
India Aadhaar Card Extraction Example
=====================================

Usage:
  npx ts-node india-aadhaar-card.ts <document-path>
  npx ts-node india-aadhaar-card.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node india-aadhaar-card.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/india-aadhaar-card
...
(async () => {
  try {
    if (args.length === 1) {
      await extractIndiaAadhaarCard(args[0]);
    } else {
