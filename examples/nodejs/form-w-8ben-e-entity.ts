/**
 * Form W-8BEN-E (Entity) Data Extraction Example
 *
 * Extract structured data from form w-8ben-e (entity) documents using AI-powered extraction.
 *
 * Extract foreign entity status, FATCA classification, and treaty benefits from IRS Form W-8BEN-E
 *
 * Fields Extracted:
 *   - Form Revision
 *   - Entity Name
 ...
 * Try it online: https://structurify.ai/extract/form-w-8ben-e-entity
 *
...
async function extractFormW8BenEEntity(filePath: string) {
  console.log('='.repeat(60));
  console.log('Form W-8BEN-E (Entity) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Form W-8BEN-E (Entity) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Form W-8BEN-E (Entity) - SDK Example',
    templateId: 'ptpl_w8ben_e',
  });
  ...
  // Save results
  const outputCsv = `form-w-8ben-e-entity_extracted.csv`;
  const outputJson = `form-w-8ben-e-entity_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Form W-8BEN-E (Entity) - Batch Processing',
    templateId: 'ptpl_w8ben_e',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Form W-8BEN-E (Entity) Extraction Example
=========================================

Usage:
  npx ts-node form-w-8ben-e-entity.ts <document-path>
  npx ts-node form-w-8ben-e-entity.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node form-w-8ben-e-entity.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/form-w-8ben-e-entity
...
(async () => {
  try {
    if (args.length === 1) {
      await extractFormW8BenEEntity(args[0]);
    } else {
