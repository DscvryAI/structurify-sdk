/**
 * Form W-4 (Employee Withholding) Data Extraction Example
 *
 * Extract structured data from form w-4 (employee withholding) documents using AI-powered extraction.
 *
 * Extract withholding elections, filing status, and adjustments from IRS Form W-4
 *
 * Fields Extracted:
 *   - Form Year
 *   - First Name
 ...
 *
 * Industry: HR / Payroll
 * Try it online: https://structurify.ai/extract/form-w-4-employee-withholding
 *
...
async function extractFormW4EmployeeWithholding(filePath: string) {
  console.log('='.repeat(60));
  console.log('Form W-4 (Employee Withholding) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Form W-4 (Employee Withholding) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Form W-4 (Employee Withholding) - SDK Example',
    templateId: 'ptpl_w4_form',
  });
  ...
  // Save results
  const outputCsv = `form-w-4-employee-withholding_extracted.csv`;
  const outputJson = `form-w-4-employee-withholding_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Form W-4 (Employee Withholding) - Batch Processing',
    templateId: 'ptpl_w4_form',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Form W-4 (Employee Withholding) Extraction Example
==================================================

Usage:
  npx ts-node form-w-4-employee-withholding.ts <document-path>
  npx ts-node form-w-4-employee-withholding.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node form-w-4-employee-withholding.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/form-w-4-employee-withholding
...
(async () => {
  try {
    if (args.length === 1) {
      await extractFormW4EmployeeWithholding(args[0]);
    } else {
