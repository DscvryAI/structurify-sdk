/**
 * Non-Conformance Report (NCR) Data Extraction Example
 *
 * Extract structured data from non-conformance report (ncr) documents using AI-powered extraction.
 *
 * Extract data from quality non-conformance and corrective action reports
 *
 * Fields Extracted:
 *   - NCR Number
 *   - Date Opened
 ...
 * Try it online: https://structurify.ai/extract/non-conformance-report-ncr
 *
...
async function extractNonConformanceReportNcr(filePath: string) {
  console.log('='.repeat(60));
  console.log('Non-Conformance Report (NCR) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Non-Conformance Report (NCR) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Non-Conformance Report (NCR) - SDK Example',
    templateId: 'ptpl_ncr',
  });
  ...
  // Save results
  const outputCsv = `non-conformance-report-ncr_extracted.csv`;
  const outputJson = `non-conformance-report-ncr_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Non-Conformance Report (NCR) - Batch Processing',
    templateId: 'ptpl_ncr',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Non-Conformance Report (NCR) Extraction Example
===============================================

Usage:
  npx ts-node non-conformance-report-ncr.ts <document-path>
  npx ts-node non-conformance-report-ncr.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node non-conformance-report-ncr.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/non-conformance-report-ncr
...
(async () => {
  try {
    if (args.length === 1) {
      await extractNonConformanceReportNcr(args[0]);
    } else {
