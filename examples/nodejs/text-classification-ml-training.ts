/**
 * Text Classification - ML Training Data Extraction Example
 *
 * Extract structured data from text classification - ml training documents using AI-powered extraction.
 *
 * Label text documents with categories, sentiment, entities, and attributes for NLP model training
 *
 * Fields Extracted:
 *   - Primary Category
 ...
 * Try it online: https://structurify.ai/extract/text-classification-ml-training
 *
...
async function extractTextClassificationMlTraining(filePath: string) {
  console.log('='.repeat(60));
  console.log('Text Classification - ML Training Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Text Classification - ML Training template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Text Classification - ML Training - SDK Example',
    templateId: 'ptpl_text_classification',
  });
  ...
  // Save results
  const outputCsv = `text-classification-ml-training_extracted.csv`;
  const outputJson = `text-classification-ml-training_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Text Classification - ML Training - Batch Processing',
    templateId: 'ptpl_text_classification',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Text Classification - ML Training Extraction Example
====================================================

Usage:
  npx ts-node text-classification-ml-training.ts <document-path>
  npx ts-node text-classification-ml-training.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node text-classification-ml-training.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/text-classification-ml-training
...
(async () => {
  try {
    if (args.length === 1) {
      await extractTextClassificationMlTraining(args[0]);
    } else {
