/**
 * Meeting Notes Data Extraction Example
 *
 * Extract structured data from meeting notes documents using AI-powered extraction.
 *
 ...
 *
 * Industry: General
 * Try it online: https://structurify.ai/extract/meeting-notes
 *
...
async function extractMeetingNotes(filePath: string) {
  console.log('='.repeat(60));
  console.log('Meeting Notes Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Meeting Notes template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Meeting Notes - SDK Example',
    templateId: 'ptpl_meeting_notes',
  });
  ...
  // Save results
  const outputCsv = `meeting-notes_extracted.csv`;
  const outputJson = `meeting-notes_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Meeting Notes - Batch Processing',
    templateId: 'ptpl_meeting_notes',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Meeting Notes Extraction Example
================================

Usage:
  npx ts-node meeting-notes.ts <document-path>
  npx ts-node meeting-notes.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node meeting-notes.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/meeting-notes
...
(async () => {
  try {
    if (args.length === 1) {
      await extractMeetingNotes(args[0]);
    } else {
