/**
 * SOC 2 Report Data Extraction Example
 *
 * Extract structured data from soc 2 report documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/soc-2-report
 *
...
async function extractSoc2Report(filePath: string) {
  console.log('='.repeat(60));
  console.log('SOC 2 Report Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the SOC 2 Report template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'SOC 2 Report - SDK Example',
    templateId: 'ptpl_soc2_report',
  });
  ...
  // Save results
  const outputCsv = `soc-2-report_extracted.csv`;
  const outputJson = `soc-2-report_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'SOC 2 Report - Batch Processing',
    templateId: 'ptpl_soc2_report',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
SOC 2 Report Extraction Example
===============================

Usage:
  npx ts-node soc-2-report.ts <document-path>
  npx ts-node soc-2-report.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node soc-2-report.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/soc-2-report
...
(async () => {
  try {
    if (args.length === 1) {
      await extractSoc2Report(args[0]);
    } else {
