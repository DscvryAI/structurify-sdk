/**
 * Batch Production Record Data Extraction Example
 *
 * Extract structured data from batch production record documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/batch-production-record
 *
...
async function extractBatchProductionRecord(filePath: string) {
  console.log('='.repeat(60));
  console.log('Batch Production Record Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Batch Production Record template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Batch Production Record - SDK Example',
    templateId: 'ptpl_batch_record',
  });
  ...
  // Save results
  const outputCsv = `batch-production-record_extracted.csv`;
  const outputJson = `batch-production-record_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Batch Production Record - Batch Processing',
    templateId: 'ptpl_batch_record',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Batch Production Record Extraction Example
==========================================

Usage:
  npx ts-node batch-production-record.ts <document-path>
  npx ts-node batch-production-record.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node batch-production-record.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/batch-production-record
...
(async () => {
  try {
    if (args.length === 1) {
      await extractBatchProductionRecord(args[0]);
    } else {
