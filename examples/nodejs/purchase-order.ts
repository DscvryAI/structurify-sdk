/**
 * Purchase Order Data Extraction Example
 *
 * Extract structured data from purchase order documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/purchase-order
 *
...
async function extractPurchaseOrder(filePath: string) {
  console.log('='.repeat(60));
  console.log('Purchase Order Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Purchase Order template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Purchase Order - SDK Example',
    templateId: 'ptpl_purchase_order',
  });
  ...
  // Save results
  const outputCsv = `purchase-order_extracted.csv`;
  const outputJson = `purchase-order_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Purchase Order - Batch Processing',
    templateId: 'ptpl_purchase_order',
  });
  console.log(`Project created: ${project.id}`);
...
Usage:
  npx ts-node purchase-order.ts <document-path>
  npx ts-node purchase-order.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node purchase-order.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/purchase-order
...
(async () => {
  try {
    if (args.length === 1) {
      await extractPurchaseOrder(args[0]);
    } else {
