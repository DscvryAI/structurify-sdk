/**
 * UK Driving Licence Data Extraction Example
 *
 * Extract structured data from uk driving licence documents using AI-powered extraction.
 *
 * Extract data from UK driving licences
 *
 * Fields Extracted:
 *   - License Number
 *   - Surname
 ...
 * Try it online: https://structurify.ai/extract/uk-driving-licence
 *
...
async function extractUkDrivingLicence(filePath: string) {
  console.log('='.repeat(60));
  console.log('UK Driving Licence Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the UK Driving Licence template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'UK Driving Licence - SDK Example',
    templateId: 'ptpl_uk_drivers_license',
  });
  ...
  // Save results
  const outputCsv = `uk-driving-licence_extracted.csv`;
  const outputJson = `uk-driving-licence_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'UK Driving Licence - Batch Processing',
    templateId: 'ptpl_uk_drivers_license',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
UK Driving Licence Extraction Example
=====================================

Usage:
  npx ts-node uk-driving-licence.ts <document-path>
  npx ts-node uk-driving-licence.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node uk-driving-licence.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/uk-driving-licence
...
(async () => {
  try {
    if (args.length === 1) {
      await extractUkDrivingLicence(args[0]);
    } else {
