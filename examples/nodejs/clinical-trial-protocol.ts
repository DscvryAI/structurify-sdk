/**
 * Clinical Trial Protocol Data Extraction Example
 *
 * Extract structured data from clinical trial protocol documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/clinical-trial-protocol
 *
...
async function extractClinicalTrialProtocol(filePath: string) {
  console.log('='.repeat(60));
  console.log('Clinical Trial Protocol Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Clinical Trial Protocol template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Clinical Trial Protocol - SDK Example',
    templateId: 'ptpl_clinical_trial_protocol',
  });
  ...
  // Save results
  const outputCsv = `clinical-trial-protocol_extracted.csv`;
  const outputJson = `clinical-trial-protocol_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Clinical Trial Protocol - Batch Processing',
    templateId: 'ptpl_clinical_trial_protocol',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Clinical Trial Protocol Extraction Example
==========================================

Usage:
  npx ts-node clinical-trial-protocol.ts <document-path>
  npx ts-node clinical-trial-protocol.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node clinical-trial-protocol.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/clinical-trial-protocol
...
(async () => {
  try {
    if (args.length === 1) {
      await extractClinicalTrialProtocol(args[0]);
    } else {
