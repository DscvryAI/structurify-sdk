/**
 * Azure Invoice Data Extraction Example
 *
 * Extract structured data from azure invoice documents using AI-powered extraction.
 *
 * Extract billing details, service charges, and usage data from Microsoft Azure invoices and billing statements
 *
 * Fields Extracted:
 *   - Invoice Number
 *   - Invoice Date
 ...
 * Try it online: https://structurify.ai/extract/azure-invoice
 *
...
async function extractAzureInvoice(filePath: string) {
  console.log('='.repeat(60));
  console.log('Azure Invoice Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Azure Invoice template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Azure Invoice - SDK Example',
    templateId: 'ptpl_azure_invoice',
  });
  ...
  // Save results
  const outputCsv = `azure-invoice_extracted.csv`;
  const outputJson = `azure-invoice_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Azure Invoice - Batch Processing',
    templateId: 'ptpl_azure_invoice',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Azure Invoice Extraction Example
================================

Usage:
  npx ts-node azure-invoice.ts <document-path>
  npx ts-node azure-invoice.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node azure-invoice.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/azure-invoice
...
(async () => {
  try {
    if (args.length === 1) {
      await extractAzureInvoice(args[0]);
    } else {
