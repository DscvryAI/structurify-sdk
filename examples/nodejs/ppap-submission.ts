/**
 * PPAP Submission Data Extraction Example
 *
 * Extract structured data from ppap submission documents using AI-powered extraction.
 *
 * Extract data from Production Part Approval Process (PPAP) packages
 *
 * Fields Extracted:
 *   - Part Number
 *   - Part Name
 ...
 * Try it online: https://structurify.ai/extract/ppap-submission
 *
...
async function extractPpapSubmission(filePath: string) {
  console.log('='.repeat(60));
  console.log('PPAP Submission Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the PPAP Submission template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'PPAP Submission - SDK Example',
    templateId: 'ptpl_ppap',
  });
  ...
  // Save results
  const outputCsv = `ppap-submission_extracted.csv`;
  const outputJson = `ppap-submission_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'PPAP Submission - Batch Processing',
    templateId: 'ptpl_ppap',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
PPAP Submission Extraction Example
==================================

Usage:
  npx ts-node ppap-submission.ts <document-path>
  npx ts-node ppap-submission.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node ppap-submission.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/ppap-submission
...
(async () => {
  try {
    if (args.length === 1) {
      await extractPpapSubmission(args[0]);
    } else {
