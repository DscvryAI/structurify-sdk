/**
 * AWS Invoice Data Extraction Example
 *
 * Extract structured data from aws invoice documents using AI-powered extraction.
 *
 * Extract billing details, service charges, and usage data from Amazon Web Services invoices and billing statements
 *
 * Fields Extracted:
 *   - Invoice Number
 *   - Invoice Date
 ...
 * Try it online: https://structurify.ai/extract/aws-invoice
 *
...
async function extractAwsInvoice(filePath: string) {
  console.log('='.repeat(60));
  console.log('AWS Invoice Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the AWS Invoice template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'AWS Invoice - SDK Example',
    templateId: 'ptpl_aws_invoice',
  });
  ...
  // Save results
  const outputCsv = `aws-invoice_extracted.csv`;
  const outputJson = `aws-invoice_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'AWS Invoice - Batch Processing',
    templateId: 'ptpl_aws_invoice',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
AWS Invoice Extraction Example
==============================

Usage:
  npx ts-node aws-invoice.ts <document-path>
  npx ts-node aws-invoice.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node aws-invoice.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/aws-invoice
...
(async () => {
  try {
    if (args.length === 1) {
      await extractAwsInvoice(args[0]);
    } else {
