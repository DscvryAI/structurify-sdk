/**
 * Loan Application Data Extraction Example
 *
 * Extract structured data from loan application documents using AI-powered extraction.
 *
 * Extract data from loan application forms
 *
 * Fields Extracted:
 *   - Applicant Name
 *   - Co-Applicant Name
 ...
 * Try it online: https://structurify.ai/extract/loan-application
 *
...
async function extractLoanApplication(filePath: string) {
  console.log('='.repeat(60));
  console.log('Loan Application Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Loan Application template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Loan Application - SDK Example',
    templateId: 'ptpl_loan_application',
  });
  ...
  // Save results
  const outputCsv = `loan-application_extracted.csv`;
  const outputJson = `loan-application_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Loan Application - Batch Processing',
    templateId: 'ptpl_loan_application',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Loan Application Extraction Example
===================================

Usage:
  npx ts-node loan-application.ts <document-path>
  npx ts-node loan-application.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node loan-application.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/loan-application
...
(async () => {
  try {
    if (args.length === 1) {
      await extractLoanApplication(args[0]);
    } else {
