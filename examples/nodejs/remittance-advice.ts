/**
 * Remittance Advice Data Extraction Example
 *
 * Extract structured data from remittance advice documents using AI-powered extraction.
 *
 * Extract payment remittance details
 *
 * Fields Extracted:
 *   - Payer Name
 *   - Payer Account
 ...
 * Try it online: https://structurify.ai/extract/remittance-advice
 *
...
async function extractRemittanceAdvice(filePath: string) {
  console.log('='.repeat(60));
  console.log('Remittance Advice Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Remittance Advice template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Remittance Advice - SDK Example',
    templateId: 'ptpl_remittance_advice',
  });
  ...
  // Save results
  const outputCsv = `remittance-advice_extracted.csv`;
  const outputJson = `remittance-advice_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Remittance Advice - Batch Processing',
    templateId: 'ptpl_remittance_advice',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Remittance Advice Extraction Example
====================================

Usage:
  npx ts-node remittance-advice.ts <document-path>
  npx ts-node remittance-advice.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node remittance-advice.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/remittance-advice
...
(async () => {
  try {
    if (args.length === 1) {
      await extractRemittanceAdvice(args[0]);
    } else {
