/**
 * UK VAT Invoice Data Extraction Example
 *
 * Extract structured data from uk vat invoice documents using AI-powered extraction.
 *
 * Extract data from UK VAT invoices
 *
 * Fields Extracted:
 *   - Supplier Name
 *   - Supplier VAT Number
 ...
 * Try it online: https://structurify.ai/extract/uk-vat-invoice
 *
...
async function extractUkVatInvoice(filePath: string) {
  console.log('='.repeat(60));
  console.log('UK VAT Invoice Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the UK VAT Invoice template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'UK VAT Invoice - SDK Example',
    templateId: 'ptpl_uk_vat_invoice',
  });
  ...
  // Save results
  const outputCsv = `uk-vat-invoice_extracted.csv`;
  const outputJson = `uk-vat-invoice_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'UK VAT Invoice - Batch Processing',
    templateId: 'ptpl_uk_vat_invoice',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
UK VAT Invoice Extraction Example
=================================

Usage:
  npx ts-node uk-vat-invoice.ts <document-path>
  npx ts-node uk-vat-invoice.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node uk-vat-invoice.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/uk-vat-invoice
...
(async () => {
  try {
    if (args.length === 1) {
      await extractUkVatInvoice(args[0]);
    } else {
