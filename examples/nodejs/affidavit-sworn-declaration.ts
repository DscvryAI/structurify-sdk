/**
 * Affidavit / Sworn Declaration Data Extraction Example
 *
 * Extract structured data from affidavit / sworn declaration documents using AI-powered extraction.
 *
 * Extract declarant information, statements, and attestation details from affidavits and sworn declarations
 *
 * Fields Extracted:
 *   - Document Type
 *   - Document Title
 ...
 *
 * Industry: Legal
 * Try it online: https://structurify.ai/extract/affidavit-sworn-declaration
 *
...
async function extractAffidavitSwornDeclaration(filePath: string) {
  console.log('='.repeat(60));
  console.log('Affidavit / Sworn Declaration Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Affidavit / Sworn Declaration template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Affidavit / Sworn Declaration - SDK Example',
    templateId: 'ptpl_affidavit',
  });
  ...
  // Save results
  const outputCsv = `affidavit-sworn-declaration_extracted.csv`;
  const outputJson = `affidavit-sworn-declaration_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Affidavit / Sworn Declaration - Batch Processing',
    templateId: 'ptpl_affidavit',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Affidavit / Sworn Declaration Extraction Example
================================================

Usage:
  npx ts-node affidavit-sworn-declaration.ts <document-path>
  npx ts-node affidavit-sworn-declaration.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node affidavit-sworn-declaration.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/affidavit-sworn-declaration
...
(async () => {
  try {
    if (args.length === 1) {
      await extractAffidavitSwornDeclaration(args[0]);
    } else {
