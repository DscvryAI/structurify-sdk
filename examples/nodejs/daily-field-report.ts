/**
 * Daily Field Report Data Extraction Example
 *
 * Extract structured data from daily field report documents using AI-powered extraction.
 *
 * Extract data from construction daily field/site reports
 *
 * Fields Extracted:
 *   - Report Date
 *   - Project Name
 ...
 * Try it online: https://structurify.ai/extract/daily-field-report
 *
...
async function extractDailyFieldReport(filePath: string) {
  console.log('='.repeat(60));
  console.log('Daily Field Report Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Daily Field Report template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Daily Field Report - SDK Example',
    templateId: 'ptpl_daily_field_report',
  });
  ...
  // Save results
  const outputCsv = `daily-field-report_extracted.csv`;
  const outputJson = `daily-field-report_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Daily Field Report - Batch Processing',
    templateId: 'ptpl_daily_field_report',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Daily Field Report Extraction Example
=====================================

Usage:
  npx ts-node daily-field-report.ts <document-path>
  npx ts-node daily-field-report.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node daily-field-report.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/daily-field-report
...
(async () => {
  try {
    if (args.length === 1) {
      await extractDailyFieldReport(args[0]);
    } else {
