/**
 * Vehicle Certificate of Title Data Extraction Example
 *
 * Extract structured data from vehicle certificate of title documents using AI-powered extraction.
 *
 * Extract data from US vehicle titles
 *
 * Fields Extracted:
 *   - State
 *   - Title Number
 ...
 *
 * Industry: Legal
 * Try it online: https://structurify.ai/extract/vehicle-certificate-of-title
 *
...
async function extractVehicleCertificateOfTitle(filePath: string) {
  console.log('='.repeat(60));
  console.log('Vehicle Certificate of Title Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Vehicle Certificate of Title template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Vehicle Certificate of Title - SDK Example',
    templateId: 'ptpl_vehicle_title',
  });
  ...
  // Save results
  const outputCsv = `vehicle-certificate-of-title_extracted.csv`;
  const outputJson = `vehicle-certificate-of-title_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Vehicle Certificate of Title - Batch Processing',
    templateId: 'ptpl_vehicle_title',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Vehicle Certificate of Title Extraction Example
===============================================

Usage:
  npx ts-node vehicle-certificate-of-title.ts <document-path>
  npx ts-node vehicle-certificate-of-title.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node vehicle-certificate-of-title.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/vehicle-certificate-of-title
...
(async () => {
  try {
    if (args.length === 1) {
      await extractVehicleCertificateOfTitle(args[0]);
    } else {
