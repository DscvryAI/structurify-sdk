/**
 * Health Insurance Card US Data Extraction Example
 *
 * Extract structured data from health insurance card us documents using AI-powered extraction.
 *
 * Extract data from US health insurance cards
 *
 * Fields Extracted:
 *   - Insurance Company
 *   - Plan Name
 ...
 *
 * Industry: Healthcare
 * Try it online: https://structurify.ai/extract/health-insurance-card-us
 *
...
async function extractHealthInsuranceCardUs(filePath: string) {
  console.log('='.repeat(60));
  console.log('Health Insurance Card US Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Health Insurance Card US template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Health Insurance Card US - SDK Example',
    templateId: 'ptpl_health_insurance_card',
  });
  ...
  // Save results
  const outputCsv = `health-insurance-card-us_extracted.csv`;
  const outputJson = `health-insurance-card-us_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Health Insurance Card US - Batch Processing',
    templateId: 'ptpl_health_insurance_card',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Health Insurance Card US Extraction Example
===========================================

Usage:
  npx ts-node health-insurance-card-us.ts <document-path>
  npx ts-node health-insurance-card-us.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node health-insurance-card-us.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/health-insurance-card-us
...
(async () => {
  try {
    if (args.length === 1) {
      await extractHealthInsuranceCardUs(args[0]);
    } else {
