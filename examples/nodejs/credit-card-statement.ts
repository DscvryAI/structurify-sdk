/**
 * Credit Card Statement Data Extraction Example
 *
 * Extract structured data from credit card statement documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/credit-card-statement
 *
...
async function extractCreditCardStatement(filePath: string) {
  console.log('='.repeat(60));
  console.log('Credit Card Statement Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Credit Card Statement template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Credit Card Statement - SDK Example',
    templateId: 'ptpl_credit_card_statement',
  });
  ...
  // Save results
  const outputCsv = `credit-card-statement_extracted.csv`;
  const outputJson = `credit-card-statement_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Credit Card Statement - Batch Processing',
    templateId: 'ptpl_credit_card_statement',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Credit Card Statement Extraction Example
========================================

Usage:
  npx ts-node credit-card-statement.ts <document-path>
  npx ts-node credit-card-statement.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node credit-card-statement.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/credit-card-statement
...
(async () => {
  try {
    if (args.length === 1) {
      await extractCreditCardStatement(args[0]);
    } else {
