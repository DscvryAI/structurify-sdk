/**
 * Brasil - Extrato FGTS Data Extraction Example
 *
 * Extract structured data from brasil - extrato fgts documents using AI-powered extraction.
 *
 * Extrair dados do extrato do Fundo de Garantia do Tempo de Serviço (FGTS) para auditoria e conferência de depósitos
 *
 * Fields Extracted:
 *   - Nome Trabalhador
 *   - CPF
 ...
 * Try it online: https://structurify.ai/extract/brasil-extrato-fgts
 *
...
async function extractBrasilExtratoFgts(filePath: string) {
  console.log('='.repeat(60));
  console.log('Brasil - Extrato FGTS Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Brasil - Extrato FGTS template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Brasil - Extrato FGTS - SDK Example',
    templateId: 'ptpl_brazil_fgts',
  });
  ...
  // Save results
  const outputCsv = `brasil-extrato-fgts_extracted.csv`;
  const outputJson = `brasil-extrato-fgts_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Brasil - Extrato FGTS - Batch Processing',
    templateId: 'ptpl_brazil_fgts',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Brasil - Extrato FGTS Extraction Example
========================================

Usage:
  npx ts-node brasil-extrato-fgts.ts <document-path>
  npx ts-node brasil-extrato-fgts.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node brasil-extrato-fgts.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/brasil-extrato-fgts
...
(async () => {
  try {
    if (args.length === 1) {
      await extractBrasilExtratoFgts(args[0]);
    } else {
