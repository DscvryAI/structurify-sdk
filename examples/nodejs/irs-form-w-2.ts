/**
 * IRS Form W-2 Data Extraction Example
 *
 * Extract structured data from irs form w-2 documents using AI-powered extraction.
 *
 * Extract data from W-2 Wage and Tax Statements
 *
 * Fields Extracted:
 *   - Tax Year
 *   - Employee SSN
 ...
 * Try it online: https://structurify.ai/extract/irs-form-w-2
 *
...
async function extractIrsFormW2(filePath: string) {
  console.log('='.repeat(60));
  console.log('IRS Form W-2 Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the IRS Form W-2 template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'IRS Form W-2 - SDK Example',
    templateId: 'ptpl_w2_form',
  });
  ...
  // Save results
  const outputCsv = `irs-form-w-2_extracted.csv`;
  const outputJson = `irs-form-w-2_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'IRS Form W-2 - Batch Processing',
    templateId: 'ptpl_w2_form',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
IRS Form W-2 Extraction Example
===============================

Usage:
  npx ts-node irs-form-w-2.ts <document-path>
  npx ts-node irs-form-w-2.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node irs-form-w-2.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/irs-form-w-2
