/**
 * ACORD 25 - Certificate of Liability Insurance Data Extraction Example
 *
 * Extract structured data from acord 25 - certificate of liability insurance documents using AI-powered extraction.
 *
 * Extract data from ACORD 25 certificates of insurance
 *
 * Fields Extracted:
 *   - Producer Name
 *   - Producer Phone
 ...
 *
 * Industry: Insurance
 * Try it online: https://structurify.ai/extract/acord-25-certificate-of-liability-insurance
 *
...
async function extractAcord25CertificateOfLiabilityInsurance(filePath: string) {
  console.log('='.repeat(60));
  console.log('ACORD 25 - Certificate of Liability Insurance Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the ACORD 25 - Certificate of Liability Insurance template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'ACORD 25 - Certificate of Liability Insurance - SDK Example',
    templateId: 'ptpl_acord_25',
  });
  ...
  // Save results
  const outputCsv = `acord-25-certificate-of-liability-insurance_extracted.csv`;
  const outputJson = `acord-25-certificate-of-liability-insurance_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'ACORD 25 - Certificate of Liability Insurance - Batch Processing',
    templateId: 'ptpl_acord_25',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
ACORD 25 - Certificate of Liability Insurance Extraction Example
================================================================

Usage:
  npx ts-node acord-25-certificate-of-liability-insurance.ts <document-path>
  npx ts-node acord-25-certificate-of-liability-insurance.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node acord-25-certificate-of-liability-insurance.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/acord-25-certificate-of-liability-insurance
...
(async () => {
  try {
    if (args.length === 1) {
      await extractAcord25CertificateOfLiabilityInsurance(args[0]);
    } else {
