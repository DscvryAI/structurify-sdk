/**
 * Employment Contract Data Extraction Example
 *
 * Extract structured data from employment contract documents using AI-powered extraction.
 *
 * Extract terms from employment agreements
 *
 * Fields Extracted:
 *   - Employee Name
 *   - Position
 ...
 *
 * Industry: HR
 * Try it online: https://structurify.ai/extract/employment-contract
 *
...
async function extractEmploymentContract(filePath: string) {
  console.log('='.repeat(60));
  console.log('Employment Contract Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Employment Contract template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Employment Contract - SDK Example',
    templateId: 'ptpl_employment_contract',
  });
  ...
  // Save results
  const outputCsv = `employment-contract_extracted.csv`;
  const outputJson = `employment-contract_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Employment Contract - Batch Processing',
    templateId: 'ptpl_employment_contract',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Employment Contract Extraction Example
======================================

Usage:
  npx ts-node employment-contract.ts <document-path>
  npx ts-node employment-contract.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node employment-contract.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/employment-contract
...
(async () => {
  try {
    if (args.length === 1) {
      await extractEmploymentContract(args[0]);
    } else {
