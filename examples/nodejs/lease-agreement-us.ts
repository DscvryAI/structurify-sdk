/**
 * Lease Agreement US Data Extraction Example
 *
 * Extract structured data from lease agreement us documents using AI-powered extraction.
 *
 * Extract data from US residential lease agreements
 *
 * Fields Extracted:
 *   - Landlord Name
 *   - Tenant Name
 ...
 *
 * Industry: Legal
 * Try it online: https://structurify.ai/extract/lease-agreement-us
 *
...
async function extractLeaseAgreementUs(filePath: string) {
  console.log('='.repeat(60));
  console.log('Lease Agreement US Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Lease Agreement US template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Lease Agreement US - SDK Example',
    templateId: 'ptpl_lease_agreement_us',
  });
  ...
  // Save results
  const outputCsv = `lease-agreement-us_extracted.csv`;
  const outputJson = `lease-agreement-us_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Lease Agreement US - Batch Processing',
    templateId: 'ptpl_lease_agreement_us',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Lease Agreement US Extraction Example
=====================================

Usage:
  npx ts-node lease-agreement-us.ts <document-path>
  npx ts-node lease-agreement-us.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node lease-agreement-us.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/lease-agreement-us
...
(async () => {
  try {
    if (args.length === 1) {
      await extractLeaseAgreementUs(args[0]);
    } else {
