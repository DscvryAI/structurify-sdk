/**
 * Supplier Qualification Questionnaire Data Extraction Example
 *
 * Extract structured data from supplier qualification questionnaire documents using AI-powered extraction.
 *
 * Extract data from supplier qualification/onboarding questionnaires
 *
 * Fields Extracted:
 *   - Company Name
 *   - DBA Name
 ...
 * Try it online: https://structurify.ai/extract/supplier-qualification-questionnaire
 *
...
async function extractSupplierQualificationQuestionnaire(filePath: string) {
  console.log('='.repeat(60));
  console.log('Supplier Qualification Questionnaire Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Supplier Qualification Questionnaire template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Supplier Qualification Questionnaire - SDK Example',
    templateId: 'ptpl_supplier_questionnaire',
  });
  ...
  // Save results
  const outputCsv = `supplier-qualification-questionnaire_extracted.csv`;
  const outputJson = `supplier-qualification-questionnaire_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Supplier Qualification Questionnaire - Batch Processing',
    templateId: 'ptpl_supplier_questionnaire',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Supplier Qualification Questionnaire Extraction Example
=======================================================

Usage:
  npx ts-node supplier-qualification-questionnaire.ts <document-path>
  npx ts-node supplier-qualification-questionnaire.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node supplier-qualification-questionnaire.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/supplier-qualification-questionnaire
...
(async () => {
  try {
    if (args.length === 1) {
      await extractSupplierQualificationQuestionnaire(args[0]);
    } else {
