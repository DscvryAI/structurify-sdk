/**
 * Job Hazard Analysis (JHA) Data Extraction Example
 *
 * Extract structured data from job hazard analysis (jha) documents using AI-powered extraction.
 *
 * Extract hazard assessments from Job Hazard Analysis forms for safety compliance
 *
 * Fields Extracted:
 *   - Project Name
 *   - Project Number
 ...
 * Try it online: https://structurify.ai/extract/job-hazard-analysis-jha
 *
...
async function extractJobHazardAnalysisJha(filePath: string) {
  console.log('='.repeat(60));
  console.log('Job Hazard Analysis (JHA) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Job Hazard Analysis (JHA) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Job Hazard Analysis (JHA) - SDK Example',
    templateId: 'ptpl_jha',
  });
  ...
  // Save results
  const outputCsv = `job-hazard-analysis-jha_extracted.csv`;
  const outputJson = `job-hazard-analysis-jha_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Job Hazard Analysis (JHA) - Batch Processing',
    templateId: 'ptpl_jha',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Job Hazard Analysis (JHA) Extraction Example
============================================

Usage:
  npx ts-node job-hazard-analysis-jha.ts <document-path>
  npx ts-node job-hazard-analysis-jha.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node job-hazard-analysis-jha.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/job-hazard-analysis-jha
...
(async () => {
  try {
    if (args.length === 1) {
      await extractJobHazardAnalysisJha(args[0]);
    } else {
