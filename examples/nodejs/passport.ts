/**
 * Passport Data Extraction Example
 *
 * Extract structured data from passport documents using AI-powered extraction.
 *
 * Extract data from international passports
 *
 * Fields Extracted:
 *   - Document Type
 *   - Country Code
 ...
 * Try it online: https://structurify.ai/extract/passport
 *
...
async function extractPassport(filePath: string) {
  console.log('='.repeat(60));
  console.log('Passport Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Passport template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Passport - SDK Example',
    templateId: 'ptpl_passport',
  });
  ...
  // Save results
  const outputCsv = `passport_extracted.csv`;
  const outputJson = `passport_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Passport - Batch Processing',
    templateId: 'ptpl_passport',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Passport Extraction Example
===========================

Usage:
  npx ts-node passport.ts <document-path>
  npx ts-node passport.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node passport.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/passport
