/**
 * Drilling Mud Report Data Extraction Example
 *
 * Extract structured data from drilling mud report documents using AI-powered extraction.
 *
 * Extract drilling fluid data from mud reports (Baroid, MI-SWACO, Newpark, etc.)
 *
 * Fields Extracted:
 *   - Report Date
 *   - Operator
 ...
 *   - Field/Block
 *   - County/Parish/Offshore Area
 *   - State/Province/Country
 *   - Spud Date
 *   - Current Depth
 *
 * Industry: Oil & Gas
 * Try it online: https://structurify.ai/extract/drilling-mud-report
 *
...
async function extractDrillingMudReport(filePath: string) {
  console.log('='.repeat(60));
  console.log('Drilling Mud Report Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Drilling Mud Report template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Drilling Mud Report - SDK Example',
    templateId: 'ptpl_drilling_mud_report',
  });
  ...
  // Save results
  const outputCsv = `drilling-mud-report_extracted.csv`;
  const outputJson = `drilling-mud-report_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Drilling Mud Report - Batch Processing',
    templateId: 'ptpl_drilling_mud_report',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Drilling Mud Report Extraction Example
======================================

Usage:
  npx ts-node drilling-mud-report.ts <document-path>
  npx ts-node drilling-mud-report.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node drilling-mud-report.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/drilling-mud-report
...
(async () => {
  try {
    if (args.length === 1) {
      await extractDrillingMudReport(args[0]);
    } else {
