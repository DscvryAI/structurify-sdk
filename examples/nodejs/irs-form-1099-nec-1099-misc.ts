/**
 * IRS Form 1099-NEC / 1099-MISC Data Extraction Example
 *
 * Extract structured data from irs form 1099-nec / 1099-misc documents using AI-powered extraction.
 *
 * Extract data from 1099 tax forms for contractor payments
 *
 * Fields Extracted:
 *   - Form Type
 *   - Tax Year
 ...
 * Try it online: https://structurify.ai/extract/irs-form-1099-nec-1099-misc
 *
...
async function extractIrsForm1099Nec1099Misc(filePath: string) {
  console.log('='.repeat(60));
  console.log('IRS Form 1099-NEC / 1099-MISC Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the IRS Form 1099-NEC / 1099-MISC template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'IRS Form 1099-NEC / 1099-MISC - SDK Example',
    templateId: 'ptpl_1099_form',
  });
  ...
  // Save results
  const outputCsv = `irs-form-1099-nec-1099-misc_extracted.csv`;
  const outputJson = `irs-form-1099-nec-1099-misc_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'IRS Form 1099-NEC / 1099-MISC - Batch Processing',
    templateId: 'ptpl_1099_form',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
IRS Form 1099-NEC / 1099-MISC Extraction Example
================================================

Usage:
  npx ts-node irs-form-1099-nec-1099-misc.ts <document-path>
  npx ts-node irs-form-1099-nec-1099-misc.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node irs-form-1099-nec-1099-misc.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/irs-form-1099-nec-1099-misc
...
(async () => {
  try {
    if (args.length === 1) {
      await extractIrsForm1099Nec1099Misc(args[0]);
    } else {
