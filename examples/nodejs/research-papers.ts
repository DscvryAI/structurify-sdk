/**
 * Research Papers Data Extraction Example
 *
 * Extract structured data from research papers documents using AI-powered extraction.
 *
 ...
 *
 * Industry: Research
 * Try it online: https://structurify.ai/extract/research-papers
 *
...
async function extractResearchPapers(filePath: string) {
  console.log('='.repeat(60));
  console.log('Research Papers Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Research Papers template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Research Papers - SDK Example',
    templateId: 'ptpl_research',
  });
  ...
  // Save results
  const outputCsv = `research-papers_extracted.csv`;
  const outputJson = `research-papers_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Research Papers - Batch Processing',
    templateId: 'ptpl_research',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Research Papers Extraction Example
==================================

Usage:
  npx ts-node research-papers.ts <document-path>
  npx ts-node research-papers.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node research-papers.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/research-papers
...
(async () => {
  try {
    if (args.length === 1) {
      await extractResearchPapers(args[0]);
    } else {
