/**
 * Certificate of Origin Data Extraction Example
 *
 * Extract structured data from certificate of origin documents using AI-powered extraction.
 *
 * Extract data from certificates of origin for customs and trade compliance
 *
 * Fields Extracted:
 *   - Certificate Number
 *   - Certificate Type
 ...
 * Try it online: https://structurify.ai/extract/certificate-of-origin
 *
...
async function extractCertificateOfOrigin(filePath: string) {
  console.log('='.repeat(60));
  console.log('Certificate of Origin Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Certificate of Origin template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Certificate of Origin - SDK Example',
    templateId: 'ptpl_certificate_of_origin',
  });
  ...
  // Save results
  const outputCsv = `certificate-of-origin_extracted.csv`;
  const outputJson = `certificate-of-origin_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Certificate of Origin - Batch Processing',
    templateId: 'ptpl_certificate_of_origin',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Certificate of Origin Extraction Example
========================================

Usage:
  npx ts-node certificate-of-origin.ts <document-path>
  npx ts-node certificate-of-origin.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node certificate-of-origin.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/certificate-of-origin
...
(async () => {
  try {
    if (args.length === 1) {
      await extractCertificateOfOrigin(args[0]);
    } else {
