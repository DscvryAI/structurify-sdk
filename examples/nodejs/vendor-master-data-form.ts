/**
 * Vendor Master Data Form Data Extraction Example
 *
 * Extract structured data from vendor master data form documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/vendor-master-data-form
 *
...
async function extractVendorMasterDataForm(filePath: string) {
  console.log('='.repeat(60));
  console.log('Vendor Master Data Form Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Vendor Master Data Form template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Vendor Master Data Form - SDK Example',
    templateId: 'ptpl_vendor_master',
  });
  ...
  // Save results
  const outputCsv = `vendor-master-data-form_extracted.csv`;
  const outputJson = `vendor-master-data-form_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Vendor Master Data Form - Batch Processing',
    templateId: 'ptpl_vendor_master',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Vendor Master Data Form Extraction Example
==========================================

Usage:
  npx ts-node vendor-master-data-form.ts <document-path>
  npx ts-node vendor-master-data-form.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node vendor-master-data-form.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/vendor-master-data-form
...
(async () => {
  try {
    if (args.length === 1) {
      await extractVendorMasterDataForm(args[0]);
    } else {
