/**
 * Product Catalog - Image to Listing Data Extraction Example
 *
 * Extract structured data from product catalog - image to listing documents using AI-powered extraction.
 *
 * Generate product descriptions, tags, and attributes from product images for e-commerce listings
 *
 * Fields Extracted:
 *   - Product Title
 *   - Short Description
 ...
 * Try it online: https://structurify.ai/extract/product-catalog-image-to-listing
 *
...
async function extractProductCatalogImageToListing(filePath: string) {
  console.log('='.repeat(60));
  console.log('Product Catalog - Image to Listing Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Product Catalog - Image to Listing template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Product Catalog - Image to Listing - SDK Example',
    templateId: 'ptpl_product_catalog',
  });
  ...
  // Save results
  const outputCsv = `product-catalog-image-to-listing_extracted.csv`;
  const outputJson = `product-catalog-image-to-listing_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Product Catalog - Image to Listing - Batch Processing',
    templateId: 'ptpl_product_catalog',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Product Catalog - Image to Listing Extraction Example
=====================================================

Usage:
  npx ts-node product-catalog-image-to-listing.ts <document-path>
  npx ts-node product-catalog-image-to-listing.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node product-catalog-image-to-listing.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/product-catalog-image-to-listing
...
(async () => {
  try {
    if (args.length === 1) {
      await extractProductCatalogImageToListing(args[0]);
    } else {
