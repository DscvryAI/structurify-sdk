/**
 * Commercial Lease Abstraction Data Extraction Example
 *
 * Extract structured data from commercial lease abstraction documents using AI-powered extraction.
 *
 * Extract comprehensive terms from commercial real estate leases for due diligence and asset management
 *
 * Fields Extracted:
 *   - Lease Type
 *   - Landlord
 ...
 * Try it online: https://structurify.ai/extract/commercial-lease-abstraction
 *
...
async function extractCommercialLeaseAbstraction(filePath: string) {
  console.log('='.repeat(60));
  console.log('Commercial Lease Abstraction Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Commercial Lease Abstraction template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Commercial Lease Abstraction - SDK Example',
    templateId: 'ptpl_commercial_lease',
  });
  ...
  // Save results
  const outputCsv = `commercial-lease-abstraction_extracted.csv`;
  const outputJson = `commercial-lease-abstraction_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Commercial Lease Abstraction - Batch Processing',
    templateId: 'ptpl_commercial_lease',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Commercial Lease Abstraction Extraction Example
===============================================

Usage:
  npx ts-node commercial-lease-abstraction.ts <document-path>
  npx ts-node commercial-lease-abstraction.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node commercial-lease-abstraction.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/commercial-lease-abstraction
...
(async () => {
  try {
    if (args.length === 1) {
      await extractCommercialLeaseAbstraction(args[0]);
    } else {
