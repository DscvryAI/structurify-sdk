/**
 * Construction Submittal Data Extraction Example
 *
 * Extract structured data from construction submittal documents using AI-powered extraction.
 *
 * Extract data from construction submittals
 *
 * Fields Extracted:
 *   - Submittal Number
 *   - Revision
 ...
 * Try it online: https://structurify.ai/extract/construction-submittal
 *
...
async function extractConstructionSubmittal(filePath: string) {
  console.log('='.repeat(60));
  console.log('Construction Submittal Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Construction Submittal template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Construction Submittal - SDK Example',
    templateId: 'ptpl_submittal',
  });
  ...
  // Save results
  const outputCsv = `construction-submittal_extracted.csv`;
  const outputJson = `construction-submittal_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Construction Submittal - Batch Processing',
    templateId: 'ptpl_submittal',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Construction Submittal Extraction Example
=========================================

Usage:
  npx ts-node construction-submittal.ts <document-path>
  npx ts-node construction-submittal.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node construction-submittal.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/construction-submittal
...
(async () => {
  try {
    if (args.length === 1) {
      await extractConstructionSubmittal(args[0]);
    } else {
