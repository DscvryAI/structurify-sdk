/**
 * Security Incident / Breach Report Data Extraction Example
 *
 * Extract structured data from security incident / breach report documents using AI-powered extraction.
 *
 ...
 * Try it online: https://structurify.ai/extract/security-incident-breach-report
 *
...
async function extractSecurityIncidentBreachReport(filePath: string) {
  console.log('='.repeat(60));
  console.log('Security Incident / Breach Report Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Security Incident / Breach Report template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Security Incident / Breach Report - SDK Example',
    templateId: 'ptpl_incident_report',
  });
  ...
  // Save results
  const outputCsv = `security-incident-breach-report_extracted.csv`;
  const outputJson = `security-incident-breach-report_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Security Incident / Breach Report - Batch Processing',
    templateId: 'ptpl_incident_report',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Security Incident / Breach Report Extraction Example
====================================================

Usage:
  npx ts-node security-incident-breach-report.ts <document-path>
  npx ts-node security-incident-breach-report.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node security-incident-breach-report.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/security-incident-breach-report
...
(async () => {
  try {
    if (args.length === 1) {
      await extractSecurityIncidentBreachReport(args[0]);
    } else {
