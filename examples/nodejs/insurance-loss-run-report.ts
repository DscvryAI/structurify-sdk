/**
 * Insurance Loss Run Report Data Extraction Example
 *
 * Extract structured data from insurance loss run report documents using AI-powered extraction.
 *
 * Extract claim history from loss run reports for underwriting and renewal quoting
 *
 * Fields Extracted:
 *   - Insured Name
 *   - Policy Number
 ...
 *
 * Industry: Insurance
 * Try it online: https://structurify.ai/extract/insurance-loss-run-report
 *
...
async function extractInsuranceLossRunReport(filePath: string) {
  console.log('='.repeat(60));
  console.log('Insurance Loss Run Report Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Insurance Loss Run Report template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Insurance Loss Run Report - SDK Example',
    templateId: 'ptpl_loss_run',
  });
  ...
  // Save results
  const outputCsv = `insurance-loss-run-report_extracted.csv`;
  const outputJson = `insurance-loss-run-report_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Insurance Loss Run Report - Batch Processing',
    templateId: 'ptpl_loss_run',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Insurance Loss Run Report Extraction Example
============================================

Usage:
  npx ts-node insurance-loss-run-report.ts <document-path>
  npx ts-node insurance-loss-run-report.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node insurance-loss-run-report.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/insurance-loss-run-report
...
(async () => {
  try {
    if (args.length === 1) {
      await extractInsuranceLossRunReport(args[0]);
    } else {
