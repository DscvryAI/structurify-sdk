/**
 * AFE - Authorization for Expenditure Data Extraction Example
 *
 * Extract structured data from afe - authorization for expenditure documents using AI-powered extraction.
 *
 * Extract data from Oil & Gas Authorization for Expenditure documents for JIB and cost tracking
 *
 * Fields Extracted:
 *   - AFE Number
 *   - AFE Date
 ...
 *
 * Industry: Oil & Gas
 * Try it online: https://structurify.ai/extract/afe-authorization-for-expenditure
 *
...
async function extractAfeAuthorizationForExpenditure(filePath: string) {
  console.log('='.repeat(60));
  console.log('AFE - Authorization for Expenditure Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the AFE - Authorization for Expenditure template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'AFE - Authorization for Expenditure - SDK Example',
    templateId: 'ptpl_afe',
  });
  ...
  // Save results
  const outputCsv = `afe-authorization-for-expenditure_extracted.csv`;
  const outputJson = `afe-authorization-for-expenditure_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'AFE - Authorization for Expenditure - Batch Processing',
    templateId: 'ptpl_afe',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
AFE - Authorization for Expenditure Extraction Example
======================================================

Usage:
  npx ts-node afe-authorization-for-expenditure.ts <document-path>
  npx ts-node afe-authorization-for-expenditure.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node afe-authorization-for-expenditure.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/afe-authorization-for-expenditure
...
(async () => {
  try {
    if (args.length === 1) {
      await extractAfeAuthorizationForExpenditure(args[0]);
    } else {
