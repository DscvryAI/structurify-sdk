/**
 * Closing Disclosure (TRID) Data Extraction Example
 *
 * Extract structured data from closing disclosure (trid) documents using AI-powered extraction.
 *
 * Extract final loan terms, actual fees, and settlement figures from CFPB Closing Disclosure for mortgage closing
 *
 * Fields Extracted:
 *   - Closing Date
 *   - Disbursement Date
 ...
 * Try it online: https://structurify.ai/extract/closing-disclosure-trid
 *
...
async function extractClosingDisclosureTrid(filePath: string) {
  console.log('='.repeat(60));
  console.log('Closing Disclosure (TRID) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Closing Disclosure (TRID) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Closing Disclosure (TRID) - SDK Example',
    templateId: 'ptpl_closing_disclosure',
  });
  ...
  // Save results
  const outputCsv = `closing-disclosure-trid_extracted.csv`;
  const outputJson = `closing-disclosure-trid_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Closing Disclosure (TRID) - Batch Processing',
    templateId: 'ptpl_closing_disclosure',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Closing Disclosure (TRID) Extraction Example
============================================

Usage:
  npx ts-node closing-disclosure-trid.ts <document-path>
  npx ts-node closing-disclosure-trid.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node closing-disclosure-trid.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/closing-disclosure-trid
...
(async () => {
  try {
    if (args.length === 1) {
      await extractClosingDisclosureTrid(args[0]);
    } else {
