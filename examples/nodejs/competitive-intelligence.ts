/**
 * Competitive Intelligence Data Extraction Example
 *
 * Extract structured data from competitive intelligence documents using AI-powered extraction.
 *
 * Extract product features, pricing, and positioning from competitor screenshots, websites, and marketing materials
 *
 * Fields Extracted:
 *   - Competitor Name
 *   - Source Type
 ...
 * Try it online: https://structurify.ai/extract/competitive-intelligence
 *
...
async function extractCompetitiveIntelligence(filePath: string) {
  console.log('='.repeat(60));
  console.log('Competitive Intelligence Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Competitive Intelligence template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Competitive Intelligence - SDK Example',
    templateId: 'ptpl_competitive_intel',
  });
  ...
  // Save results
  const outputCsv = `competitive-intelligence_extracted.csv`;
  const outputJson = `competitive-intelligence_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Competitive Intelligence - Batch Processing',
    templateId: 'ptpl_competitive_intel',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Competitive Intelligence Extraction Example
===========================================

Usage:
  npx ts-node competitive-intelligence.ts <document-path>
  npx ts-node competitive-intelligence.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node competitive-intelligence.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/competitive-intelligence
...
(async () => {
  try {
    if (args.length === 1) {
      await extractCompetitiveIntelligence(args[0]);
    } else {
