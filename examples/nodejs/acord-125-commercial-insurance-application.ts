/**
 * ACORD 125 - Commercial Insurance Application Data Extraction Example
 *
 * Extract structured data from acord 125 - commercial insurance application documents using AI-powered extraction.
 *
 * Extract data from ACORD 125 commercial insurance applications
 *
 * Fields Extracted:
 *   - Applicant Name
 *   - DBA Name
 ...
 *
 * Industry: Insurance
 * Try it online: https://structurify.ai/extract/acord-125-commercial-insurance-application
 *
...
async function extractAcord125CommercialInsuranceApplication(filePath: string) {
  console.log('='.repeat(60));
  console.log('ACORD 125 - Commercial Insurance Application Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the ACORD 125 - Commercial Insurance Application template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'ACORD 125 - Commercial Insurance Application - SDK Example',
    templateId: 'ptpl_acord_125',
  });
  ...
  // Save results
  const outputCsv = `acord-125-commercial-insurance-application_extracted.csv`;
  const outputJson = `acord-125-commercial-insurance-application_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'ACORD 125 - Commercial Insurance Application - Batch Processing',
    templateId: 'ptpl_acord_125',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
ACORD 125 - Commercial Insurance Application Extraction Example
===============================================================

Usage:
  npx ts-node acord-125-commercial-insurance-application.ts <document-path>
  npx ts-node acord-125-commercial-insurance-application.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node acord-125-commercial-insurance-application.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/acord-125-commercial-insurance-application
...
(async () => {
  try {
    if (args.length === 1) {
      await extractAcord125CommercialInsuranceApplication(args[0]);
    } else {
