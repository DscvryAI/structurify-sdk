/**
 * Resume Screening Data Extraction Example
 *
 * Extract structured data from resume screening documents using AI-powered extraction.
 *
 * Extract candidate information from resumes for ATS integration and screening
 *
 * Fields Extracted:
 *   - Candidate Name
 *   - Email
 ...
 *
 * Industry: HR
 * Try it online: https://structurify.ai/extract/resume-screening
 *
...
async function extractResumeScreening(filePath: string) {
  console.log('='.repeat(60));
  console.log('Resume Screening Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Resume Screening template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Resume Screening - SDK Example',
    templateId: 'ptpl_resume',
  });
  ...
  // Save results
  const outputCsv = `resume-screening_extracted.csv`;
  const outputJson = `resume-screening_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Resume Screening - Batch Processing',
    templateId: 'ptpl_resume',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Resume Screening Extraction Example
===================================

Usage:
  npx ts-node resume-screening.ts <document-path>
  npx ts-node resume-screening.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node resume-screening.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/resume-screening
...
(async () => {
  try {
    if (args.length === 1) {
      await extractResumeScreening(args[0]);
    } else {
