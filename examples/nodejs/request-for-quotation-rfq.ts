/**
 * Request for Quotation (RFQ) Data Extraction Example
 *
 ...
 * Try it online: https://structurify.ai/extract/request-for-quotation-rfq
 *
...
async function extractRequestForQuotationRfq(filePath: string) {
  console.log('='.repeat(60));
  console.log('Request for Quotation (RFQ) Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Request for Quotation (RFQ) template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Request for Quotation (RFQ) - SDK Example',
    templateId: 'ptpl_rfq_document',
  });
  ...
  // Save results
  const outputCsv = `request-for-quotation-rfq_extracted.csv`;
  const outputJson = `request-for-quotation-rfq_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Request for Quotation (RFQ) - Batch Processing',
    templateId: 'ptpl_rfq_document',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Request for Quotation (RFQ) Extraction Example
==============================================

Usage:
  npx ts-node request-for-quotation-rfq.ts <document-path>
  npx ts-node request-for-quotation-rfq.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node request-for-quotation-rfq.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/request-for-quotation-rfq
...
(async () => {
  try {
    if (args.length === 1) {
      await extractRequestForQuotationRfq(args[0]);
    } else {
