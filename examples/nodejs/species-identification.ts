/**
 * Species Identification Data Extraction Example
 *
 * Extract structured data from species identification documents using AI-powered extraction.
 *
 * Identify wildlife species and detect injuries from images
 *
 * Fields Extracted:
 *   - Common Name
 *   - Family
 ...
 *   - Behavior
 *
 * Industry: Wildlife Science
 * Try it online: https://structurify.ai/extract/species-identification
 *
...
async function extractSpeciesIdentification(filePath: string) {
  console.log('='.repeat(60));
  console.log('Species Identification Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Species Identification template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Species Identification - SDK Example',
    templateId: 'ptpl_species_identification',
  });
  ...
  // Save results
  const outputCsv = `species-identification_extracted.csv`;
  const outputJson = `species-identification_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Species Identification - Batch Processing',
    templateId: 'ptpl_species_identification',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Species Identification Extraction Example
=========================================

Usage:
  npx ts-node species-identification.ts <document-path>
  npx ts-node species-identification.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node species-identification.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/species-identification
...
(async () => {
  try {
    if (args.length === 1) {
      await extractSpeciesIdentification(args[0]);
    } else {
