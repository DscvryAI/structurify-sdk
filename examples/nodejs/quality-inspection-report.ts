/**
 * Quality Inspection Report Data Extraction Example
 *
 * Extract structured data from quality inspection report documents using AI-powered extraction.
 *
 * Extract data from dimensional inspection and CMM reports
 *
 * Fields Extracted:
 *   - Report Number
 *   - Inspection Date
 ...
 * Try it online: https://structurify.ai/extract/quality-inspection-report
 *
...
async function extractQualityInspectionReport(filePath: string) {
  console.log('='.repeat(60));
  console.log('Quality Inspection Report Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Quality Inspection Report template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Quality Inspection Report - SDK Example',
    templateId: 'ptpl_inspection_report',
  });
  ...
  // Save results
  const outputCsv = `quality-inspection-report_extracted.csv`;
  const outputJson = `quality-inspection-report_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Quality Inspection Report - Batch Processing',
    templateId: 'ptpl_inspection_report',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Quality Inspection Report Extraction Example
============================================

Usage:
  npx ts-node quality-inspection-report.ts <document-path>
  npx ts-node quality-inspection-report.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node quality-inspection-report.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/quality-inspection-report
...
(async () => {
  try {
    if (args.length === 1) {
      await extractQualityInspectionReport(args[0]);
    } else {
