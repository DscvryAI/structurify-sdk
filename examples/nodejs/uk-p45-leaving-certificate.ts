/**
 * UK P45 Leaving Certificate Data Extraction Example
 *
 * Extract structured data from uk p45 leaving certificate documents using AI-powered extraction.
 *
 * Extract data from UK P45 forms
 *
 * Fields Extracted:
 *   - Employee Name
 *   - NI Number
 ...
 *
 * Industry: Government
 * Try it online: https://structurify.ai/extract/uk-p45-leaving-certificate
 *
...
async function extractUkP45LeavingCertificate(filePath: string) {
  console.log('='.repeat(60));
  console.log('UK P45 Leaving Certificate Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the UK P45 Leaving Certificate template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'UK P45 Leaving Certificate - SDK Example',
    templateId: 'ptpl_uk_p45',
  });
  ...
  // Save results
  const outputCsv = `uk-p45-leaving-certificate_extracted.csv`;
  const outputJson = `uk-p45-leaving-certificate_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'UK P45 Leaving Certificate - Batch Processing',
    templateId: 'ptpl_uk_p45',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
UK P45 Leaving Certificate Extraction Example
=============================================

Usage:
  npx ts-node uk-p45-leaving-certificate.ts <document-path>
  npx ts-node uk-p45-leaving-certificate.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node uk-p45-leaving-certificate.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/uk-p45-leaving-certificate
...
(async () => {
  try {
    if (args.length === 1) {
      await extractUkP45LeavingCertificate(args[0]);
    } else {
