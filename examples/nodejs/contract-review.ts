/**
 * Contract Review Data Extraction Example
 *
 * Extract structured data from contract review documents using AI-powered extraction.
 *
 * Extract key terms, obligations, and risk factors from contracts and agreements
 *
 * Fields Extracted:
 *   - Contract Type
 *   - Parties
 ...
 *   - Governing Law
 *   - Dispute Resolution
 *   - Key Obligations - Party 1
 *   - Key Obligations - Party 2
 *   - Payment Terms
 *
 * Industry: Legal
 * Try it online: https://structurify.ai/extract/contract-review
 *
...
async function extractContractReview(filePath: string) {
  console.log('='.repeat(60));
  console.log('Contract Review Extraction');
  console.log('='.repeat(60));

  // Step 1: Create a project with the Contract Review template
  console.log('\n1. Creating project...');
  const project = await client.projects.create({
    name: 'Contract Review - SDK Example',
    templateId: 'ptpl_contract_review',
  });
  ...
  // Save results
  const outputCsv = `contract-review_extracted.csv`;
  const outputJson = `contract-review_extracted.json`;
...
async function extractBatch(filePaths: string[]) {
  ...
  // Create project
  const project = await client.projects.create({
    name: 'Contract Review - Batch Processing',
    templateId: 'ptpl_contract_review',
  });
  console.log(`Project created: ${project.id}`);
...
if (args.length === 0) {
  console.log(`
Contract Review Extraction Example
==================================

Usage:
  npx ts-node contract-review.ts <document-path>
  npx ts-node contract-review.ts <doc1> <doc2> <doc3>  # Batch mode

Example:
  npx ts-node contract-review.ts ./my-document.pdf

Environment:
  STRUCTURIFY_API_KEY - Your API key from https://app.structurify.ai

Learn more:
  - Online tool: https://structurify.ai/extract/contract-review
...
(async () => {
  try {
    if (args.length === 1) {
      await extractContractReview(args[0]);
    } else {
